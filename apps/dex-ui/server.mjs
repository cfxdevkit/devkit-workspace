...
const DIST_TOKEN_ICON_OVERRIDES_PATH = join(
	DIST_DIR,
	"token-icon-overrides.json",
);
const POOL_IMPORT_PRESETS_PATH = join(PUBLIC_DIR, "pool-import-presets.json");
const DIST_POOL_IMPORT_PRESETS_PATH = join(
	DIST_DIR,
...
const WORKSPACE_FILESERVER =
	process.env.WORKSPACE_FILESERVER ?? "http://127.0.0.1:7749";
...
function parseSiweMessage(message) {
	...
	for (const line of lines) {
		if (line.startsWith("URI: ")) result.uri = line.slice(5);
			...
			result.chainId = Number(line.slice(10));
...
async function _devkitFetch(path) {
	const r = await fetch(`${DEVKIT_URL}/api${path}`, {
...
function readPoolImportPresets() {
	try {
		...
		if (
			!Array.isArray(data?.selectedPoolAddresses) ||
			data.selectedPoolAddresses.length === 0
		) {
			return {
...
const server = createServer(async (req, res) => {
	...
	try {
		...
		if (pathname === "/api/dex/token-icon-overrides") {
			if (req.method === "POST") {
				try {
					...
					const icons = Array.isArray(current.icons)
		...
		if (pathname === "/api/dex/token-icon-upload" && req.method === "POST") {
			try {
				...
				const address =
					typeof body.address === "string" ? body.address.toLowerCase() : "";
				const fileName =
					typeof body.fileName === "string" ? body.fileName : "icon.png";
		...
		if (req.method === "GET" && pathname.startsWith("/assets/tokens/")) {
			...
			try {
				const r = await fetch(`${WORKSPACE_FILESERVER}${safePath}`, {
