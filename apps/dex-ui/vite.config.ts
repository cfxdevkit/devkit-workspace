...
const CONTRACT_REGISTRY_PATH = resolve(
	WORKSPACE_ROOT,
	".devkit-contracts.json",
);
const CONTRACT_ARTIFACTS_DIR = resolve(
	WORKSPACE_ROOT,
	"packages",
	"contracts",
	"artifacts",
);
const SHARED_ASSETS_DIR = resolve(WORKSPACE_ROOT, "assets");
	...
	return {
		...
		configureServer(server: ViteDevServer) {
			server.middlewares.use(
				async (req: IncomingMessage, res: DevResponse, next) => {
...
function devkitContractsPlugin(): Plugin {
	const filePath = CONTRACT_REGISTRY_PATH;
	return {
...
function localAssetsPlugin(): Plugin {
	const artifactsDir = CONTRACT_ARTIFACTS_DIR;
	const assetsDir = SHARED_ASSETS_DIR;
	const tokenIconOverridesPath = resolve(
		__dirname,
		"public/token-icon-overrides.json",
	);
	const poolImportPresetsPath = resolve(
		__dirname,
		"public/pool-import-presets.json",
	);
		...
		try {
			...
			return (catalog.pools ?? [])
				.filter(
					(pool) =>
						pool.isWcfxPair &&
						!isStablecoinAddress(pool.baseToken?.address ?? "") &&
						!isStablecoinAddress(pool.quoteToken?.address ?? ""),
				)
				.sort((left, right) => {
					if ((right.reserveUsd ?? 0) !== (left.reserveUsd ?? 0))
	...
	function readTokenIconOverrides(): TokenIconOverridesFile {
		const empty: TokenIconOverridesFile = {
			version: 1,
		...
		try {
			...
			const icons = Array.isArray(parsed.icons)
				? parsed.icons
						.filter(
							(entry): entry is TokenIconOverride =>
								isRecord(entry) &&
	...
	function readPoolImportPresets(): PoolImportPresetsFile {
		try {
			...
			return {
				...
				chainId: 1030,
				updatedAt: new Date().toISOString(),
				selectedPoolAddresses: getSuggestedPoolAddresses(),
			};
		}
	}
	...
	function writeTokenIconFile(address: string, dataUrl: string) {
		...
		writeFileSync(
			resolve(tokenIconFilesDir, fileName),
			...
			server.middlewares.use(
				async (req: IncomingMessage, res: DevResponse, next) => {
						...
						if (name) {
							...
							if (existsSync(artPath)) {
								try {
									...
									res.writeHead(200, { "Content-Type": "application/json" });
									res.end(
										JSON.stringify({ abi: art.abi, bytecode: art.bytecode }),
									);
									return;
								} catch {
									/* fall through */
					...
					if (req.url === "/api/dex/contracts" && req.method === "POST") {
						...
						const contractsPath = CONTRACT_REGISTRY_PATH;
						const parsed = existsSync(contractsPath)
							? JSON.parse(readFileSync(contractsPath, "utf-8"))
							: [];
						const existing: ContractEntry[] = Array.isArray(parsed)
							? parsed
									.filter(
										(entry): entry is ContractEntry =>
											isRecord(entry) && typeof entry.id === "string",
									)
									.map((entry) => ({ ...entry }))
							: [];
						const chainValue =
							typeof chain === "string" && chain.length > 0 ? chain : "evm";
						...
						if (req.method === "POST") {
							try {
								...
								if (!/^0x[a-f0-9]{40}$/.test(address)) {
									res.writeHead(400, { "Content-Type": "application/json" });
									res.end(
										JSON.stringify({ error: "Valid token address required" }),
									);
									return;
								}

								const current = readTokenIconOverrides();
								const icons = current.icons.filter(
					...
					if (req.url === "/api/dex/pool-import-presets") {
						if (req.method === "POST") {
							try {
								const body = await readJsonBody(req);
								const selectedPoolAddresses = Array.isArray(
					...
					if (
						req.url === "/api/dex/token-icon-upload" &&
						req.method === "POST"
					) {
						try {
							const body = await readJsonBody(req);
