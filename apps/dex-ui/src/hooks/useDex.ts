...
async function fetchTranslationTable(): Promise<Map<string, TranslationEntry>> {
	...
	try {
		const r = await fetch(
			`${import.meta.env.BASE_URL}translation-table.json?v=${Date.now()}`,
			{
				cache: "no-store",
...
async function fetchContracts(): Promise<DeployedContract[]> {
	const results = await Promise.allSettled([
		fetch(`${import.meta.env.BASE_URL}api/contracts/deployed`, {
			signal: AbortSignal.timeout(5000),
		})
			.then((r) => r.json())
			.then((d) => {
				const list =
					d?.data?.contracts ?? d?.contracts ?? (Array.isArray(d) ? d : []);
				return list as DeployedContract[];
			}),
		fetch(`${import.meta.env.BASE_URL}devkit-contracts.json`, {
...
export function useDex(): DexState {
	...
	const loadDexState = useCallback(async () => {
		...
		try {
			...
			const contractMetadataByAddress = new Map<
				string,
			...
			async function getToken(
				addr: `0x${string}`,
				contractMetadata?: DeployedContractMetadata,
			): Promise<TokenInfo> {
				...
				const resolvedMetadata =
					contractMetadata ?? contractMetadataByAddress.get(lc);
				const knownToken = resolveKnownToken(knownTokens, {
					contractAddress: lc,
					realAddress:
						lc === wethAddr.toLowerCase()
							? WCFX_MAINNET
							: (translationEntry?.realAddress ??
								resolvedMetadata?.realAddress ??
								null),
				});
				const overrideIconUrl =
					iconOverrides.get(lc)?.iconUrl ??
					(translationEntry?.realAddress
						? iconOverrides.get(translationEntry.realAddress.toLowerCase())
								?.iconUrl
						: undefined) ??
					(resolvedMetadata?.realAddress
						? iconOverrides.get(resolvedMetadata.realAddress.toLowerCase())
								?.iconUrl
						: undefined);
				const realAddress = (
					lc === wethAddr.toLowerCase()
				...
				if (lc === wethAddr.toLowerCase()) {
					...
					try {
						...
						if (knownToken) {
							...
							decimals =
								typeof resolvedMetadata.decimals === "number"
									? resolvedMetadata.decimals
									: decimals;
						}
					}
				}
			...
			for (let i = 0; i < Number(pairCount); i++) {
				...
				for (let attempt = 0; attempt < 2; attempt++) {
					try {
						...
						const [t0Addr, t1Addr, reserves, totalSupply, userLpBalance] =
							await Promise.all([
								client.readContract({
									address: pairAddress,
									abi: PAIR_ABI,
						...
						const [token0, token1] = await Promise.all([
							getToken(
								t0Addr,
								contractMetadataByAddress.get(t0Addr.toLowerCase()),
							),
							getToken(
								t1Addr,
								contractMetadataByAddress.get(t1Addr.toLowerCase()),
							),
						]);
						pools.push({
							pairAddress,
						...
						if (attempt === 0) {
							...
							console.warn(
								`[useDex] Skipped pair ${i}/${pairCount}:`,
			...
			const pairAddresses = new Set(
				pools.map((pool) => pool.pairAddress.toLowerCase()),
			);
			for (const contract of contracts) {
				const contractAddr = contract.address?.toLowerCase();
				if (
					!contractAddr ||
					seen.has(contractAddr) ||
					pairAddresses.has(contractAddr)
				)
					continue;
				if (
					contractAddr === factoryAddr.toLowerCase() ||
					contractAddr === routerAddr.toLowerCase() ||
					contractAddr === vaultAddr?.toLowerCase()
				)
					continue;
