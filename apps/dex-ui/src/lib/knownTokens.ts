...
export async function fetchKnownTokenCatalog(): Promise<
	Map<string, KnownTokenEntry>
> {
	const map = new Map<string, KnownTokenEntry>();
	try {
		const response = await fetch(
			`${import.meta.env.BASE_URL}known-tokens.json?v=${Date.now()}`,
			{
				cache: "no-store",
...
export async function fetchTokenIconOverrides(): Promise<
	Map<string, TokenIconOverrideEntry>
> {
	const map = new Map<string, TokenIconOverrideEntry>();
	try {
		const response = await fetch(
			`${import.meta.env.BASE_URL}api/dex/token-icon-overrides?v=${Date.now()}`,
			{
				cache: "no-store",
				signal: AbortSignal.timeout(3000),
			},
...
export async function fetchPoolImportPresets(): Promise<PoolImportPresetFile | null> {
	try {
		const response = await fetch(
			`${import.meta.env.BASE_URL}api/dex/pool-import-presets?v=${Date.now()}`,
			{
				cache: "no-store",
				signal: AbortSignal.timeout(3000),
			},
		...
		if (!response.ok) {
			const fallbackResponse = await fetch(
				`${import.meta.env.BASE_URL}pool-import-presets.json?v=${Date.now()}`,
				{
					cache: "no-store",
					signal: AbortSignal.timeout(3000),
				},
...
export async function savePoolImportPresets(
	selectedPoolAddresses: string[],
): Promise<PoolImportPresetFile> {
	const response = await fetch(
		`${import.meta.env.BASE_URL}api/dex/pool-import-presets`,
		{
			method: "POST",
	...
	if (!response.ok) {
		...
		throw new Error(
			(data as { error?: string }).error ??
				"Failed to save pool import presets",
		);
	}
	return (await response.json()) as PoolImportPresetFile;
}
...
export async function saveTokenIconOverride(
	address: string,
	iconUrl: string | null,
): Promise<void> {
	...
	const response = await fetch(
		`${import.meta.env.BASE_URL}api/dex/token-icon-overrides`,
		{
			method: "POST",
			headers: { "Content-Type": "application/json" },
			body: JSON.stringify({
	...
	if (!response.ok) {
		...
		throw new Error(
			(data as { error?: string }).error ??
				"Failed to save token icon override",
		);
	}
}

export async function uploadTokenIcon(
	address: string,
	file: File,
): Promise<TokenIconUploadResult> {
	...
	const response = await fetch(
		`${import.meta.env.BASE_URL}api/dex/token-icon-upload`,
		{
			method: "POST",
			headers: { "Content-Type": "application/json" },
			body: JSON.stringify({
	...
	if (!response.ok) {
		const data = await response.json().catch(() => ({}));
		throw new Error(
			(data as { error?: string }).error ?? "Failed to upload token icon",
		);
	}
...
export function resolveKnownToken(
	...
	const realAddress = options.realAddress
		? knownTokens.get(normalizeAddress(options.realAddress))
		: undefined;
	if (realAddress) return realAddress;
	return options.contractAddress
		? knownTokens.get(normalizeAddress(options.contractAddress))
		: undefined;
}
