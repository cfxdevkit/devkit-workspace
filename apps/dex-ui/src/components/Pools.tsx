	...
	return (
		<div className="flex min-w-0 items-center gap-2.5 overflow-hidden">
			{iconUrl ? (
				...
				<div className="flex h-[18px] w-[18px] shrink-0 items-center justify-center rounded-full bg-border text-[8px] font-bold uppercase text-text-secondary">
					{symbol[0]}
				</div>
			)}
...
export function Pools({ dex }: { dex: DexState }) {
	...
	const activePoolData = useMemo(
		() => dex.pools.find((pool) => pool.pairAddress === activePool) ?? null,
		[activePool, dex.pools],
	);
	const connectedPositions = dex.pools.filter(
		(pool) => pool.userLpBalance > 0n,
	).length;
	...
	const getRemoveEstimates = (pool: PoolInfo, pct: number) => {
		...
		const liquidity =
			(pool.userLpBalance * BigInt(Math.round(pct * 100))) / 10000n;
	...
	const handleRemoveLiquidity = async (pool: PoolInfo) => {
		...
		const { liquidity, amount0, amount1 } = getRemoveEstimates(
			pool,
			removePercent,
		);
		if (liquidity <= 0n) {
		...
		<div className="w-full rounded-[2rem] border border-white/5 bg-bg-secondary/40 p-6 md:p-8 backdrop-blur-2xl shadow-2xl relative overflow-hidden h-fit">
				...
				right={
					<div className="flex items-center gap-4">
						<div className="flex flex-col items-end gap-1">
							<span className="text-[8px] font-black uppercase tracking-[0.2em] text-text-secondary/30 italic">
								Network Health
							</span>
							<div className="flex gap-2">
								<div className="flex items-center gap-1.5 rounded-lg border border-white/10 bg-white/5 px-3 py-1.5 shadow-md">
									<span className="text-[9px] font-black uppercase tracking-[0.1em] text-white/80">
										{dex.pools.length} Pairs
									</span>
								</div>
			...
			{isWrongChain && (
				<div className="relative z-10 mb-6 rounded-2xl border border-warning/10 bg-warning/5 p-4 backdrop-blur-md flex items-center justify-between gap-4 px-1 animate-fade-in">
					<div className="flex items-center gap-3 px-3">
						...
						<div className="text-[10px] font-black uppercase tracking-[0.15em] text-warning/70">
							Connect to Conflux eSpace (Chain 2030) to operate.
						</div>
					</div>
					<Button
						onClick={handleSwitchChain}
						variant="secondary"
						className="!h-8 !px-4 !text-[9px] font-black uppercase tracking-[0.2em] whitespace-nowrap shadow-md"
					>
						Switch
					</Button>
				</div>
			...
			{dex.loading && (
				<div className="flex items-center gap-2 px-1 mb-4">
					...
					<p className="text-[9px] font-black uppercase tracking-[0.2em] text-text-secondary/60 animate-pulse">
						Syncing Atlas...
					</p>
				</div>
			)}
			...
			{!dex.loading && dex.pools.length === 0 && (
				<div className="relative py-16 rounded-2xl border border-dashed border-white/5 flex flex-col items-center justify-center bg-white/[0.01] mx-1">
					<div className="text-xs font-black text-white/40 uppercase tracking-[0.2em]">
						Zero Liquidity Index
					</div>
					<p className="mt-1 text-[10px] text-text-secondary/20 font-bold uppercase tracking-widest text-center italic">
						Protocol factory initialization required.
					</p>
				</div>
			)}
			...
			{dex.pools.length > 0 && (
				<div className="relative">
					<div className="mb-3 hidden grid-cols-[minmax(0,2.2fr)_minmax(180px,1fr)_minmax(220px,1.2fr)_150px] gap-6 px-6 lg:grid">
						<div className="text-left text-[8px] font-black uppercase tracking-[0.24em] text-text-secondary/55 italic">
							Pair Hierarchy
						</div>
						<div className="text-left text-[8px] font-black uppercase tracking-[0.24em] text-text-secondary/55 italic">
							Global Depth
						...
						<div className="text-right text-[8px] font-black uppercase tracking-[0.24em] text-text-secondary/55 italic">
							Control
						</div>
					</div>
					<div className="space-y-3">
						{dex.pools.map((p) => {
							...
							const tokenReserve = token0IsBase
								? Number(p.reserve1) / 10 ** p.token1.decimals
								: Number(p.reserve0) / 10 ** p.token0.decimals;
							const cfxReserve = token0IsBase
								? Number(p.reserve0) / 10 ** p.token0.decimals
								: Number(p.reserve1) / 10 ** p.token1.decimals;
							...
							return (
								<div key={p.pairAddress} className="group">
									<div
										className={`overflow-hidden rounded-2xl border transition-all duration-300 ${isOpen ? "border-accent/30 bg-white/5 shadow-xl" : "border-white/5 bg-white/[0.03] shadow-md hover:border-white/20 hover:bg-white/[0.06]"}`}
									>
										<div className="grid gap-4 px-5 py-4 lg:grid-cols-[minmax(0,2.2fr)_minmax(180px,1fr)_minmax(220px,1.2fr)_150px] lg:items-center lg:gap-6 lg:px-6">
											...
											<div className="flex flex-col gap-0.5">
												<div className="text-[7px] font-black uppercase tracking-[0.2em] text-text-secondary/60 italic">
													Global Reserves
												</div>
											...
											<div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-[minmax(0,1fr)_96px] lg:items-center">
												...
												<div className="flex flex-col gap-0.5">
													...
													<div className="font-mono text-[11px] font-black text-white/85">
														{hasPosition
															? `${getSharePct(p).toFixed(3)}%`
															: "—"}
													</div>
												</div>
											</div>
											...
											<div className="flex items-center justify-end gap-3 lg:pr-1">
												<CopyButton
													copyText={p.pairAddress}
													title={`Pair: ${p.pairAddress}\nClick to copy`}
													size="md"
													stopPropagation
													className="shadow-sm hover:scale-105"
												/>
												<Button
													onClick={() =>
														hasPosition
															? setActivePool(isOpen ? null : p.pairAddress)
															: null
													}
											...
											<div className="animate-fade-in-up border-t border-white/5 bg-black/10 p-6 backdrop-blur-xl">
												<div className="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-6">
													<div>
														...
														<p className="mt-0.5 text-[10px] text-text-secondary/40 uppercase font-black tracking-widest italic leading-none">
															Recover proportional underlying asset depth.
														</p>
													</div>
												...
												<div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
													<div className="rounded-xl border border-white/5 bg-white/5 p-4 backdrop-blur-md shadow-inner transition-all hover:border-white/10">
														<div className="text-[8px] font-black uppercase tracking-[0.2em] text-text-secondary/20 italic leading-none mb-2">
															LP Balance
														</div>
														<div className="font-mono text-xs font-black text-white/90 tracking-widest tabular-nums">
															{Number(
																formatUnits(
																	getRemoveEstimates(p, removePercent)
																		.liquidity,
																	18,
																),
															).toLocaleString()}{" "}
													...
													<div className="rounded-xl border border-white/5 bg-white/5 p-4 backdrop-blur-md shadow-inner transition-all hover:border-accent/30">
														<div className="text-[8px] font-black uppercase tracking-[0.2em] text-text-secondary/20 italic leading-none mb-2">
															Claimable {p.token0.symbol}
														</div>
														<div className="font-mono text-xs font-black text-white/90 tracking-widest tabular-nums">
															{Number(
																formatUnits(
																	getRemoveEstimates(p, removePercent).amount0,
																	p.token0.decimals,
																),
															).toLocaleString()}
														</div>
													...
													<div className="rounded-xl border border-white/5 bg-white/5 p-4 backdrop-blur-md shadow-inner transition-all hover:border-accent/30">
														<div className="text-[8px] font-black uppercase tracking-[0.2em] text-text-secondary/20 italic leading-none mb-2">
															Claimable {p.token1.symbol}
														</div>
														<div className="font-mono text-xs font-black text-white/90 tracking-widest tabular-nums">
															{Number(
																formatUnits(
																	getRemoveEstimates(p, removePercent).amount1,
																	p.token1.decimals,
																),
															).toLocaleString()}
														</div>
												...
												{status &&
													activePoolData?.pairAddress === p.pairAddress && (
														<StatusBanner
															message={status}
															tone={
