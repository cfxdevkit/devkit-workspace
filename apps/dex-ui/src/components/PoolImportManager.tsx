...
function getSuggestedPoolSelection(
...
export function PoolImportManager() {
	...
	useEffect(() => {
		...
		void Promise.all([fetchKnownTokenCatalogFile(), fetchPoolImportPresets()])
			.then(([catalog, presets]) => {
				...
				const presetSelection = (presets?.selectedPoolAddresses ?? []).map(
				...
				setSelectedPools(
					new Set(
						presetSelection.length > 0
							? presetSelection
							: getSuggestedPoolSelection(nextPools),
					),
				);
			})
			.catch((error: unknown) => {
	...
	const saveSelection = async () => {
		...
		try {
			...
			setSelectedPools(
				new Set(
					saved.selectedPoolAddresses.map((entry) => entry.toLowerCase()),
				),
			);
			setStatus(`Saved ${saved.selectedPoolAddresses.length} selected pools`);
		} catch (error) {
			setStatus(
				error instanceof Error ? error.message : "Failed to save pool preset",
			);
		} finally {
			setSaving(false);
		}
			...
			<SectionHeader
				...
				right={
					<div className="text-[8px] font-black uppercase tracking-[0.24em] text-text-secondary/30 italic">
						{selectedPools.size} pools selected • {selectedTokenCount} import
						tokens
					</div>
				}
			/>
			...
			<div className="relative grid gap-4">
				<div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_auto_auto] lg:items-center">
					...
					<Button
						variant="secondary"
						onClick={() =>
							setSelectedPools(new Set(getSuggestedPoolSelection(pools)))
						}
					>
						Reset Suggested
					</Button>
								...
								subtitleClassName="break-all whitespace-normal overflow-visible text-ellipsis-clip"
									...
									<div className="flex flex-wrap justify-end gap-2 text-[8px] font-black uppercase tracking-[0.18em] text-text-secondary/40">
										<span className="rounded-full border border-white/10 px-2 py-1">
											{formatUsd(pool.reserveUsd)} liq
										</span>
										<span className="rounded-full border border-white/10 px-2 py-1">
											{formatUsd(pool.volume24h)} 24h
										</span>
										{pool.isWcfxPair ? (
											<span className="rounded-full border border-accent/20 bg-accent/10 px-2 py-1 text-accent/80">
												WCFX
											</span>
										) : null}
									</div>
