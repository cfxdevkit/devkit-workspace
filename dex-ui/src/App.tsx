...
import { Card, DevkitStatus, FaucetWidget, MetricCard, ShellOverview, SegmentedControl } from '@cfxdevkit/ui-shared';
...
function ShellSectionButton({
  ...
  return (
    ...
    >
      <div className={`text-[13px] font-black tracking-tight transition-colors ${active ? 'text-white' : 'group-hover:text-text-primary uppercase'}`}>{label}</div>
      <div className={`mt-0.5 text-[8px] font-black uppercase tracking-[0.18em] transition-colors ${active ? 'text-accent/80' : 'text-text-secondary/30 group-hover:text-text-secondary/50'}`}>{meta}</div>
      {active && <div className="absolute bottom-0 left-0 h-0.5 w-full bg-gradient-to-r from-transparent via-accent to-transparent opacity-50" />}
    </button>
  );
}
      ...
      <main className="mx-auto flex w-full max-w-7xl flex-1 flex-col gap-5 px-5 py-6 md:py-8">
        <div className="animate-fade-in-up">
          <ShellOverview
            ...
            statusLabel={dexReady ? 'Protocol Active' : 'Synchronization Pending'}
            statusVariant={dexReady ? 'success' : 'warning'}
            metrics={
              <>
                <MetricCard label="Latency" value="Local" hint="Sub-ms updates" />
                <MetricCard label="Factory" value="Uniswap V2" hint="AMM engine" />
        ...
        <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-4 animate-fade-in-up" style={{ animationDelay: '0.1s' }}>
          <ShellSectionButton active={activeSection === 'trade'} label="Trade" meta={sectionMeta.trade} onClick={() => setActiveSection('trade')} />
          <ShellSectionButton active={activeSection === 'portfolio'} label="Portfolio" meta={sectionMeta.portfolio} onClick={() => setActiveSection('portfolio')} />
          <ShellSectionButton active={activeSection === 'pools'} label="Pools" meta={sectionMeta.pools} onClick={() => setActiveSection('pools')} />
          <ShellSectionButton active={activeSection === 'tools'} label="Tools" meta={sectionMeta.tools} onClick={() => setActiveSection('tools')} />
        </section>
            ...
            <section className="grid gap-5 xl:grid-cols-[minmax(0,320px)_minmax(0,1fr)]">
              <div className="flex flex-col gap-5">
                <Card className="border-white/5 p-5">
                  <div className="mb-5 flex items-center justify-between gap-3">
                    <div>
                      <h2 className="text-lg font-black text-white uppercase tracking-tight">Analytics</h2>
                      <p className="mt-0.5 text-[10px] text-text-secondary/60 font-black uppercase tracking-widest italic">Node Diagnostics</p>
                    </div>
                    <button
                      type="button"
                      onClick={dex.refresh}
                  ...
                  <div className="grid gap-2.5">
                    <MetricCard label="DEX Pairs" value={String(dex.pools.length)} hint="Liquidity Index" />
                    <MetricCard label="Tokens" value={String(dex.tokens.length)} hint="Tracked Assets" />
                    <MetricCard label="Status" value={dex.error ? 'Error' : dex.loading ? 'Sync' : 'Ready'} hint={dex.error ?? 'All nodes active'} variant={dex.error ? 'default' : 'success'} />
                  </div>
                </Card>
              </div>
              ...
              <div className="flex flex-col gap-5">
                ...
                <div className="transition-all duration-500">
                   {tradeAction === 'swap' ? <Swap dex={dex} /> : <AddLiquidity dex={dex} />}
                </div>
              </div>
            </section>
          )}
            ...
            <section className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_300px]">
              ...
              <div className="flex flex-col gap-5">
                <Card className="border-white/5 h-fit sticky top-24 p-5">
                  <h2 className="text-lg font-black text-white uppercase tracking-tight">Inventory</h2>
                  <p className="mt-3 text-[11px] leading-relaxed text-text-secondary/60 font-medium italic">
                    Real-time balance tracking for all deployed tokens in the local sandbox range.
                  </p>
                  <div className="mt-5 grid gap-2.5">
                    <MetricCard label="Visibility" value="Partial" hint="Local assets only" />
                    <MetricCard label="Frequency" value="10s" hint="Auto-sync active" />
                  </div>
                </Card>
              </div>
            </section>
