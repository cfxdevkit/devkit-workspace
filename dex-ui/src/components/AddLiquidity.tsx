...
import { Button, SectionHeader, StatusBanner, TradeActionBar, TradeSummaryGrid, TradeTokenField } from '@cfxdevkit/ui-shared';
...
export function AddLiquidity({ dex }: Props) {
  ...
  const refreshBalances = useCallback(async () => {
    ...
    try {
      const balanceA = tokenA.isNative
        ...
        : await publicClient.readContract({ address: tokenA.address, abi: ERC20_ABI, functionName: 'balanceOf', args: [address] });
      const balanceB = tokenB.isNative
        ? await publicClient.getBalance({ address })
        : await publicClient.readContract({ address: tokenB.address, abi: ERC20_ABI, functionName: 'balanceOf', args: [address] });
  ...
  const pairLabel = tokenA && tokenB ? `${tokenA.symbol} / ${tokenB.symbol}` : 'Pair';
  ...
  useEffect(() => {
    ...
    const poolToken0MatchesA = selectedPool.token0.address.toLowerCase() === tokenA.address.toLowerCase();
    const reserveA = Number(formatUnits(poolToken0MatchesA ? selectedPool.reserve0 : selectedPool.reserve1, tokenA.decimals));
    const reserveB = Number(formatUnits(poolToken0MatchesA ? selectedPool.reserve1 : selectedPool.reserve0, tokenB.decimals));
    if (!Number.isFinite(reserveA) || !Number.isFinite(reserveB) || reserveA <= 0 || reserveB <= 0) return;
  ...
  const handleAdd = async () => {
    ...
    if (!walletClient || !publicClient || !address || !tokenA || !tokenB || !routerAddress) return;
    if (!amountA || !amountB) {
    ...
    try {
      ...
      if (isNativeA || isNativeB) {
        ...
        const hash = await walletClient.writeContract({
          ...
          args: [tokenA.address, tokenB.address, aWei, bWei, 0n, 0n, address, deadline],
        });
        await publicClient.waitForTransactionReceipt({ hash });
      }
      ...
      setStatus(`Error: ${err instanceof Error ? err.message.slice(0, 80) : 'failed'}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  if (!isConnected || tokens.length < 2) {
    return (
      <div className="rounded-[1.5rem] border border-white/5 bg-bg-secondary/40 p-6 backdrop-blur-2xl shadow-lg relative overflow-hidden">
        ...
        <h2 className="text-xl font-black tracking-tighter text-white uppercase mb-2">Liquidity</h2>
        <p className="text-[11px] text-text-secondary/60 font-medium leading-relaxed italic opacity-80">{!isConnected ? 'Connect wallet to authorize protocol seeding.' : 'No tradable assets indexed in this environment.'}</p>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-[2rem] border border-white/5 bg-bg-secondary/40 p-6 md:p-8 backdrop-blur-2xl shadow-2xl relative overflow-visible">
      ...
      <div className="relative flex flex-col gap-2">
        <TradeTokenField label="Asset A" sideLabel={balA ? `Account: ${balA}` : 'Init Asset'} amount={amountA} onAmountChange={handleAmountAChange} tokens={tokens} selectedIndex={tokenAIdx} onTokenChange={handleTokenAChange} />
        ...
        <div className="relative z-20 -my-4 flex justify-center">
          <div className="flex h-10 w-10 items-center justify-center rounded-xl border-[4px] border-bg-secondary bg-bg-tertiary shadow-xl group">
            <span className="text-lg font-black text-accent drop-shadow-[0_0_8px_rgba(79,142,255,0.4)]">+</span>
          </div>
        </div>

        <TradeTokenField label="Asset B" sideLabel={balB ? `Account: ${balB}` : 'Init Asset'} amount={amountB} onAmountChange={handleAmountBChange} tokens={tokens} selectedIndex={tokenBIdx} onTokenChange={handleTokenBChange} />

        <div className="mt-6 flex flex-col gap-4">
          <TradeSummaryGrid
            items={[
              { label: 'Active Interface', value: tokenA?.isNative || tokenB?.isNative ? 'addLiquidityETH' : 'addLiquidity' },
              { label: 'Symmetry', value: selectedPool ? 'Balanced' : 'Manual' },
          ...
          <TradeActionBar>
            <div className="max-w-[300px] text-[9px] font-black uppercase tracking-[0.1em] leading-relaxed text-text-secondary/40 italic">
              Provisioning liquidity mints LP tokens to the authorized account instantly.
            </div>
            <Button
              onClick={isWrongChain ? handleSwitchChain : handleAdd}
              disabled={loading || (!isWrongChain && (!amountA || !amountB || tokenAIdx === tokenBIdx))}
            ...
            >
              {isWrongChain ? 'Switch to eSpace' : loading ? 'Syncing...' : tokenAIdx === tokenBIdx ? 'Invalid Pair' : 'Execute Supply'}
            </Button>
          </TradeActionBar>
        </div>
      </div>
