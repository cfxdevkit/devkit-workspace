...
import { Button, Card, SectionHeader, StatusBanner, TradeActionBar, TradeSummaryGrid, TradeTokenField } from '@cfxdevkit/ui-shared';
...
async function buildPath(
  ...
  if (dex.weth && (from.toLowerCase() === dex.weth.toLowerCase() || to.toLowerCase() === dex.weth.toLowerCase())) {
    return direct;
  }
  try {
...
function getPoolForPair(dex: DexState, from: `0x${string}`, to: `0x${string}`): PoolInfo | undefined {
...
function simulateRouteExact(dex: DexState, path: `0x${string}`[], amountIn: bigint): {
  amountOut: bigint;
  routeSymbols: string[];
  executionPrice: number | null;
  priceImpactPct: number | null;
  ...
  for (let index = 0; index < path.length - 1; index++) {
    ...
    const fromIsToken0 = pool.token0.address.toLowerCase() === from.toLowerCase();
    ...
    const amountOut = (amountInWithFee * reserveOutRaw) / (reserveInRaw * 1000n + amountInWithFee);

    const reserveIn = Number(reserveInRaw) / 10 ** inToken.decimals;
    const reserveOut = Number(reserveOutRaw) / 10 ** outToken.decimals;
    const amountInDisplay = Number(formatUnits(currentAmountIn, inToken.decimals));
    const amountOutDisplay = Number(formatUnits(amountOut, outToken.decimals));
    const spotPrice = reserveIn > 0 ? reserveOut / reserveIn : 0;
    const executionPrice = amountInDisplay > 0 ? amountOutDisplay / amountInDisplay : 0;
    const hopImpact = spotPrice > 0 ? Math.max(((spotPrice - executionPrice) / spotPrice) * 100, 0) : 0;
    ...
    hops.push({
      ...
      executionPrice: executionPrice > 0 ? `${executionPrice.toFixed(6)} ${outToken.symbol}/${inToken.symbol}` : '—',
  ...
  const totalAmountOutDisplay = Number(formatUnits(totalAmountOut, dex.tokens.find((token) => token.address.toLowerCase() === path[path.length - 1].toLowerCase())?.decimals ?? 18));
  const totalAmountInDisplay = Number(formatUnits(amountIn, dex.tokens.find((token) => token.address.toLowerCase() === path[0].toLowerCase())?.decimals ?? 18));
  const executionPrice = totalAmountInDisplay > 0 ? totalAmountOutDisplay / totalAmountInDisplay : null;
  const priceImpactPct = executionPrice != null && cumulativeSpotPrice > 0
    ? Math.max(((cumulativeSpotPrice - executionPrice) / cumulativeSpotPrice) * 100, 0)
    : null;
...
export function Swap({ dex }: { dex: DexState }) {
  ...
  const toToken = ready ? dex.tokens[toIdx >= dex.tokens.length ? 0 : toIdx] : null;
  const amountOutPreview = quoteDetails && toToken
    ? Number(formatUnits(quoteDetails.amountOut, toToken.decimals)).toLocaleString(undefined, { maximumFractionDigits: 6 })
    : '0.0';
  ...
  useEffect(() => {
    ...
    const loadBalances = async () => {
      try {
        ...
        if (cancelled) return;
        setFromBalance((Number(nextFromBalance) / 10 ** fromToken.decimals).toFixed(4));
        setToBalance((Number(nextToBalance) / 10 ** toToken.decimals).toFixed(4));
      } catch {
        if (cancelled) return;
        setFromBalance('—');
        setToBalance('—');
  ...
  useEffect(() => {
    if (!ready || !amount || !publicClient || !dex.router || !fromToken || !toToken) {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        setQuoteDetails({
          amountOut,
          route: path,
          routeSymbols: exactRoute?.routeSymbols ?? path.map((tokenAddress) => dex.tokens.find((token) => token.address.toLowerCase() === tokenAddress.toLowerCase())?.symbol ?? 'Token'),
        ...
        if (!controller.signal.aborted) {
          setQuoteDetails(null);
          setStatus(`Quote error: ${err instanceof Error ? err.message.slice(0, 80) : 'unknown'}`);
        }
      }
    }, 300);
  ...
  if (!ready || !fromToken || !toToken) {
    return (
      <Card className="rounded-[1.5rem] border-white/5 bg-bg-secondary/40 p-6 backdrop-blur-xl">
        <h2 className="text-xl font-black text-white uppercase tracking-tighter mb-2">Swap Engine</h2>
        <p className="text-text-secondary text-[11px] font-medium opacity-60 italic">Protocol synchronization pending. Ensure target contracts are deployed.</p>
      </Card>
    );
  }

  const handleSwap = async () => {
    if (!walletClient) { setStatus('Switch to Conflux eSpace Local (chain 2030)'); return; }
    if (!publicClient || !address || !dex.router) { setStatus('Wallet Not Connected'); return; }
    if (!amount) { setStatus('Amount Required'); return; }
    ...
    try {
      ...
      const path = await buildPath(fromToken.address, toToken.address, publicClient, dex);
      const amountOutMin = quoteDetails ? minOutWithSlippage(quoteDetails.amountOut, slippagePct) : 0n;
    ...
    <div className="rounded-[2rem] border border-white/5 bg-bg-secondary/40 p-6 md:p-8 backdrop-blur-2xl shadow-2xl relative overflow-visible">
      ...
      <div className="relative flex flex-col gap-2">
        ...
        <div className="absolute left-1/2 top-1/2 z-20 -translate-x-1/2 -translate-y-1/2">
          ...
          >
            <span className="text-lg transition-all duration-700 group-hover:rotate-180">↕</span>
          </button>
        </div>
      ...
      <div className="mt-6 flex flex-col gap-4">
        <TradeSummaryGrid
          items={[
            { label: 'Network Route', value: quoteDetails ? quoteDetails.routeSymbols.join(' → ') : 'Syncing Path...' },
            { label: 'Minimum Out', value: quoteDetails?.minimumReceived ?? '—' },
            { label: 'Price Impact', value: formatPct(quoteDetails?.priceImpactPct ?? null) },
            { label: 'Service Fee', value: quoteDetails ? `${quoteDetails.feePct.toFixed(2)}%` : '—', tone: 'accent' },
          ]}
        />

        <TradeActionBar>
          <div className="flex flex-col gap-1.5 text-[9px] uppercase font-black tracking-[0.1em] text-text-secondary/65">
            <div className="flex items-center gap-2 italic">Slippage Limit: <span className="text-white not-italic">{slippagePct}%</span></div>
            <div className="flex items-center gap-2 italic">Exchange Rate: <span className="text-white not-italic">{quoteDetails?.executionPrice != null ? `${quoteDetails.executionPrice.toFixed(6)} ${toToken.symbol}/${fromToken.symbol}` : '—'}</span></div>
          </div>
          <Button
            onClick={isWrongChain ? handleSwitchChain : handleSwap}
            disabled={!isConnected || (!isWrongChain && (!amount || amountOutPreview === '—'))}
          ...
          >
            {isWrongChain ? 'Switch to eSpace' : !walletClient && isConnected ? 'Authorize Wallet' : 'Execute Swap'}
          </Button>
        </TradeActionBar>
      </div>

      {status && <StatusBanner message={status} tone={status.toLowerCase().includes('error') ? 'error' : 'accent'} className="mt-4 px-5 py-3" />}
    </div>
  );
}
