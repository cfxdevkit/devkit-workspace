...
import { Card, Button, SectionHeader, StatusBanner, TradeTokenField } from '@cfxdevkit/ui-shared';
...
export function Vault({ dex }: { dex: DexState }) {
  ...
  const allTokens = useMemo<(TokenInfo & { vaultToken: `0x${string}` })[]>(() => {
    ...
    return [
      ...
      ...dex.tokens.filter((token) => !token.isNative).map((token) => ({ ...token, vaultToken: token.address })),
    ];
  }, [dex.tokens]);
  ...
  if (!isConnected || !dex.vault) {
    return (
      <Card className="relative h-fit overflow-hidden rounded-[2rem] border-white/5 bg-bg-secondary/40 p-6 shadow-2xl backdrop-blur-2xl">
        ...
        <h2 className="mb-3 text-xl font-black tracking-tight text-white uppercase">Vault Control</h2>
        <p className="text-sm font-medium leading-relaxed text-text-secondary">{!isConnected ? 'Connect developer wallet to authorize vault interactions.' : 'PayableVault protocol synchronization pending.'}</p>
        {!isConnected && (
          <div className="mt-6 rounded-[1.25rem] border border-white/10 bg-white/5 p-5 shadow-inner backdrop-blur-sm">
            <div className="flex items-center gap-3">
              ...
              <span className="text-[10px] font-black uppercase tracking-[0.2em] text-text-secondary/60 text-center">Authorization required for local state access.</span>
            </div>
          </div>
        )}
      </Card>
  ...
  const handleDeposit = async () => {
    ...
    if (!walletClient) { setStatus('Switch to Local eSpace'); return; }
    if (!publicClient || !address || !dex.vault) { setStatus('Syncing Connection...'); return; }
    ...
    <Card className="relative h-fit overflow-visible rounded-[2rem] border-white/5 bg-bg-secondary/40 p-5 md:p-6 shadow-2xl backdrop-blur-2xl">
      ...
      <div className="relative mb-8 grid gap-3 grid-cols-2">
        <div className="rounded-2xl border border-white/5 bg-white/5 p-4 backdrop-blur-md shadow-inner">
          <div className="text-[8px] font-black uppercase tracking-[0.25em] text-text-secondary/30">Active Asset</div>
           <div className="mt-1 text-base font-black uppercase tracking-tight text-white">{selected.symbol}</div>
        </div>
        <div className="rounded-2xl border border-white/5 bg-white/5 p-4 backdrop-blur-md shadow-inner">
          <div className="text-[8px] font-black uppercase tracking-[0.25em] text-text-secondary/30">Vault Credits</div>
           <div className="mt-1 text-base font-black leading-none tracking-tight text-success tabular-nums">
            {vaultBal && vaultBal !== '—'
              ? Number(vaultBal).toLocaleString(undefined, { maximumFractionDigits: 6 })
              : '0.0'}
          </div>
        </div>
      </div>
      ...
      <div className="relative flex flex-col gap-6">
        <TradeTokenField
          label="Vault Asset"
          sideLabel={vaultBal && vaultBal !== '—' ? `Credits: ${Number(vaultBal).toLocaleString(undefined, { maximumFractionDigits: 6 })}` : 'Credits: 0.0'}
          amount={amount}
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <Button 
            ...
            disabled={!isConnected || (!isWrongChain && (!walletClient || !amount))} 
            variant="primary" 
        ...
        <div className="rounded-xl border border-white/5 bg-white/5 p-3.5 shadow-inner backdrop-blur-sm">
           <div className="flex items-start gap-3">
            ...
            <p className="text-[9px] font-black uppercase tracking-[0.1em] leading-relaxed text-text-secondary/40">
              Vault allows protocol-wide settlements without redundant approvals.
            </p>
          </div>
        </div>
      </div>
      ...
      {status && (
        <StatusBanner
          message={status}
          tone={status.toLowerCase().includes('confirmed') ? 'success' : status.toLowerCase().includes('error') ? 'error' : 'accent'}
          className="mt-5"
        />
      )}
    </Card>
