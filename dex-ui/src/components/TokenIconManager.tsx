...
function getCanonicalAddress(token: { address: string; realAddress?: string | null }): string {
...
function buildItems(
  ...
  for (const token of dex.tokens) {
    ...
    const realAddress = token.realAddress ? normalizeAddress(token.realAddress) : null;
    const publicAddress = existing?.publicAddress ?? realAddress ?? (tokenAddress === canonicalAddress ? canonicalAddress : null);
    ...
    items.set(canonicalAddress, {
      ...
      name: existing?.name ?? (token.isNative ? 'Native Wrapped Asset' : 'Local Token'),
      iconUrl: token.iconUrl ?? existing?.iconUrl,
      overrideIconUrl: overrides.get(canonicalAddress)?.iconUrl ?? existing?.overrideIconUrl,
      publicAddress,
...
export function TokenIconManager({ dex }: Props) {
  const [knownTokens, setKnownTokens] = useState<Map<string, KnownTokenEntry>>(new Map());
  const [overrides, setOverrides] = useState<Map<string, TokenIconOverrideEntry>>(new Map());
  ...
  useEffect(() => {
    ...
    void Promise.all([fetchKnownTokenCatalog(), fetchTokenIconOverrides()])
      ...
      .catch((error: unknown) => {
        if (cancelled) return;
        setStatus(error instanceof Error ? error.message : 'Failed to load token icons');
      })
      .finally(() => {
  ...
  const items = useMemo(() => buildItems(knownTokens, overrides, dex), [knownTokens, overrides, dex]);
  ...
  const persistOverride = async (value: string | null) => {
    ...
    try {
      ...
      setStatus(error instanceof Error ? error.message : 'Failed to save override');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleUpload = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const uploaded = await uploadTokenIcon(selectedItem.canonicalAddress, file);
      await persistOverride(uploaded.iconUrl);
      setStatus('Uploaded icon and saved override');
    } catch (error) {
      setStatus(error instanceof Error ? error.message : 'Failed to upload icon');
      setSaving(false);
    } finally {
      event.target.value = '';
    }
  ...
  const activeIconUrl = selectedItem?.overrideIconUrl ?? selectedItem?.iconUrl ?? '';
  ...
  return (
    <div className="rounded-[2rem] border border-white/5 bg-bg-secondary/40 p-6 md:p-8 backdrop-blur-2xl shadow-2xl relative overflow-hidden">
      ...
      <input ref={fileInputRef} type="file" accept="image/png,image/jpeg,image/webp,image/gif,image/svg+xml" className="hidden" onChange={(event) => void handleUpload(event)} />
          ...
          <div className="mt-4 min-h-0 flex-1 overflow-auto pr-1">
            <div className="grid gap-2">
              {filteredItems.map((item) => {
                ...
                const active = selectedItem?.canonicalAddress === item.canonicalAddress;
                return (
                  <SelectableListItem
                    ...
                    icon={(
                      ...
                      {icon ? (
                        <img src={icon} alt={item.symbol} className="h-8 w-8 object-contain" />
                      ) : (
                        <span className="text-[9px] font-black uppercase tracking-[0.18em] text-text-secondary/40">None</span>
                      )}
                      </div>
                    )}
                    title={item.symbol}
            ...
            <div className="grid gap-5">
              <div className="flex flex-col gap-4 lg:flex-row lg:items-center">
                <div className="flex h-20 w-20 items-center justify-center rounded-[1.75rem] border border-white/10 bg-bg-primary/60">
                  {activeIconUrl ? (
                    <img src={activeIconUrl} alt={selectedItem.symbol} className="h-14 w-14 object-contain" />
                  ) : (
                    <span className="text-[10px] font-black uppercase tracking-[0.18em] text-text-secondary/35">No Icon</span>
                  )}
                </div>

                <div className="min-w-0 flex-1">
                  <div className="text-xl font-black uppercase tracking-tight text-white">{selectedItem.symbol}</div>
                  <div className="mt-1 text-xs font-medium italic text-text-secondary/60">{selectedItem.name}</div>
              ...
              <div className="grid gap-3">
                <label htmlFor={overrideInputId} className="text-[8px] font-black uppercase tracking-[0.28em] text-text-secondary/30">Override URL</label>
                <input
                  id={overrideInputId}
                  value={iconUrlInput}
                ...
                <div className="text-[10px] leading-relaxed text-text-secondary/50">
                  Save a direct URL or upload a local file. The override is stored on the canonical token address and applies to related mirror/runtime addresses automatically.
                </div>
              </div>

              <div className="grid gap-3 md:grid-cols-2">
                <div className="rounded-2xl border border-white/5 bg-bg-primary/35 px-4 py-4">
                  <div className="text-[8px] font-black uppercase tracking-[0.24em] text-text-secondary/30">Catalog Icon</div>
                  <div className="mt-2 break-all text-[10px] font-medium text-text-secondary/55">{selectedItem.iconUrl ?? 'None'}</div>
                </div>
                <div className="rounded-2xl border border-white/5 bg-bg-primary/35 px-4 py-4">
                  <div className="text-[8px] font-black uppercase tracking-[0.24em] text-text-secondary/30">Active Override</div>
                  <div className="mt-2 break-all text-[10px] font-medium text-text-secondary/55">{selectedItem.overrideIconUrl ?? 'None'}</div>
                </div>
              </div>

              <div className="rounded-2xl border border-white/5 bg-bg-primary/35 px-4 py-4">
                <div className="text-[8px] font-black uppercase tracking-[0.24em] text-text-secondary/30">Related Addresses</div>
                <div className="mt-3 grid gap-3">
                  <div className="rounded-xl border border-white/5 bg-bg-primary/40 px-3 py-3">
                    <div className="text-[8px] font-black uppercase tracking-[0.24em] text-text-secondary/30">Public Address</div>
                    <div className="mt-2 break-all text-[10px] font-medium text-text-secondary/55">{selectedItem.publicAddress ?? 'Not linked to a public canonical token'}</div>
                  </div>
                  <div className="rounded-xl border border-white/5 bg-bg-primary/40 px-3 py-3">
                    <div className="text-[8px] font-black uppercase tracking-[0.24em] text-text-secondary/30">Local Address{selectedItem.localAddresses.length === 1 ? '' : 'es'}</div>
                    <div className="mt-2 grid gap-2">
                      {selectedItem.localAddresses.length > 0 ? selectedItem.localAddresses.map((address) => (
                        <div key={address} className="break-all text-[10px] font-medium text-text-secondary/55">{address}</div>
                      )) : (
                        <div className="text-[10px] font-medium text-text-secondary/40">No local mirror address recorded</div>
                      )}
                    </div>
                  </div>
                </div>
              ...
              <div className="flex flex-wrap gap-3">
                <Button onClick={() => void persistOverride(iconUrlInput.trim() || null)} disabled={saving || !selectedItem.canonicalAddress}>
                  {saving ? 'Saving…' : 'Save Override'}
                </Button>
                <Button variant="secondary" onClick={() => fileInputRef.current?.click()} disabled={saving || !selectedItem.canonicalAddress}>
                  Upload Icon
                </Button>
                <Button variant="secondary" onClick={() => void persistOverride(null)} disabled={saving || !selectedItem.overrideIconUrl}>
                  Remove Override
                </Button>
              </div>

              {status && <StatusBanner message={status} tone={status.toLowerCase().includes('fail') || status.toLowerCase().includes('error') ? 'error' : status.toLowerCase().includes('saved') || status.toLowerCase().includes('uploaded') || status.toLowerCase().includes('removed') ? 'success' : 'accent'} className="rounded-2xl bg-bg-primary/35 text-text-secondary/45" textClassName="text-[10px] tracking-[0.16em]" />}
            </div>
          )}
        </div>
      </div>
