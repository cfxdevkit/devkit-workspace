...
function persistOperationRecord(record) {
  ...
  writeFileSync(resolve(operationsDir, `${record.operation}.latest.json`), `${JSON.stringify(record, null, 2)}\n`, 'utf8');
  appendFileSync(resolve(operationsDir, 'ledger.ndjson'), `${JSON.stringify(record)}\n`, 'utf8');
}
