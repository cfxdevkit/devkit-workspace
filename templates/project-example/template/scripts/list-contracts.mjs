...
async function readTrackedAddress() {
  const generatedAddressesPath = resolve(process.cwd(), 'dapp', 'src', 'generated', 'contracts-addresses.js');
  if (!existsSync(generatedAddressesPath)) {
    return null;
  }

  const module = await import(pathToFileURL(generatedAddressesPath).href);
  return module.CONTRACT_ADDRESSES_BY_CHAIN_ID?.[exampleCounterArtifact.chainId]?.[exampleCounterArtifact.contractName] ?? null;
}
