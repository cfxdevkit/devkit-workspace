  ...
  const checks = [
    {
      ...
      ok: existsSync(resolve(process.cwd(), 'dapp', 'src', 'generated', 'devkit-target.js')),
    },
    {
      name: 'project-network',
      path: 'dapp/src/generated/project-network.js',
      ok: existsSync(resolve(process.cwd(), 'dapp', 'src', 'generated', 'project-network.js')),
    },
    {
      name: 'contract-addresses',
      path: 'dapp/src/generated/contracts-addresses.js',
      ok: existsSync(resolve(process.cwd(), 'dapp', 'src', 'generated', 'contracts-addresses.js')),
    },
    {
      name: 'contract-artifact',
      path: 'contracts/generated/example-counter.js',
      ok: existsSync(resolve(process.cwd(), 'contracts', 'generated', 'example-counter.js')),
    },
    {
      name: 'generation-manifest',
      path: '.new-devkit/manifest.json',
      ok: existsSync(resolve(process.cwd(), '.new-devkit', 'manifest.json')),
    },
  ];
...
await runOperation('doctor', flags, async ({ step }) => {
  ...
  const result = {
    ...
    target: '{{TARGET_NAME}}',
    features: {
      baseUrl: '{{BASE_URL_ENABLED}}',
      proxy: '{{PROXY_ENABLED}}',
      codeServer: '{{CODE_SERVER_ENABLED}}',
    },
    ready,
    checks,
    message: ready
  ...
  step('summarize-readiness', { status: ready ? 'completed' : 'warning', ready });
  return result;
});
