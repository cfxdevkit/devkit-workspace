...
| `dex_manifest` | Get deployed contract addresses (factory, router02, WETH9) |
| `dex_translation_table` | Get token symbol → address mapping for all deployed tokens |
| `dex_wcfx_price` | Get current WCFX/USD price from the local DEX |
...
Step 4: dex_manifest                   → get contract addresses (factory, router02, WETH9)
Step 5: dex_translation_table          → token symbol/address map
```
...
|------|-----------|---------------|
| **Token address** | ERC-20 contract (`MirrorERC20`, `WETH9`) | `dex_translation_table` |
| **Pair address** (pool) | `UniswapV2Pair` contract | Read from factory via `blockchain_espace_call_contract` |
| **Router/Factory address** | DEX infrastructure | `dex_manifest` |
