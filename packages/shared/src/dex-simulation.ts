...
import type { VWAPPoint, FeedCache, } from './dex-feed';
import { computeSwapAmountForTargetPrice, reservesToPrice } from './dex-reserves';
...
export interface ChainAdapter {
  ...
  getReserves(pairAddress: string): Promise<{ reserve0: bigint; reserve1: bigint }>;
...
export function computeSmoothedPrice(
  ...
  if (weightByVol) {
    ...
    if (totalVol > 0) {
      return filtered.reduce((s, c) => s + c.price * Math.max(c.volume, 1), 0) / totalVol;
    }
  }
 ...
 */
export function deviationBps(currentPrice: number, targetPrice: number): number {
  if (currentPrice <= 0) return 10000; // 100% deviation if no current price
  return Math.round(Math.abs(targetPrice - currentPrice) / currentPrice * 10000);
}
...
export class SimulationEngine {
  ...
  private stats: TickStats = { ticks: 0, rebalances: 0, swapsSent: 0, injectsDone: 0, skipped: 0, errors: 0 };
  private tickCount = 0;
  ...
  static async create(params: {
    ...
    for (const token of params.feedCache.tokens) {
      ...
      const isToken0 = mapping.localAddress.toLowerCase() < params.wethAddress.toLowerCase();
      ...
      const currentPrice = reservesToPrice(tokenReserve, wethReserve, token.decimals);
  ...
  async step(feedCache: FeedCache): Promise<TickResult> {
    ...
    for (const ts of this.tokens) {
      ...
      const feedToken = feedCache.tokens.find(
        t => t.realAddress.toLowerCase() === this.getTokenRealAddress(ts, feedCache)
      );
      ...
      const priceRatio = feedToken.priceUsd > 0 ? smoothedUsd / feedToken.priceUsd : 1;
      // Use the FIRST candle's price as baseline for the ratio, since the initial
      // reserves were seeded at feedToken.priceUsd
      const targetPrice = ts.initialPrice > 0
        ? priceRatio * ts.initialPrice
        : ts.lastPrice;
      ...
      const { tokenReserve, wethReserve } = sortReserves(rawReserves, ts.isToken0);
      const currentPrice = reservesToPrice(tokenReserve, wethReserve, ts.decimals);
      ...
      try {
        if (method === 'swap') {
          await this.rebalanceViaSwap(ts, { tokenReserve, wethReserve }, targetPrice, rbResult);
        } else {
          await this.rebalanceViaInjection(ts, { tokenReserve, wethReserve }, targetPrice, rbResult);
        }
        ...
        rbResult.priceAfter = reservesToPrice(newSorted.tokenReserve, newSorted.wethReserve, ts.decimals);
  ...
  async reset(): Promise<void> {
    ...
    this.tickCount = 0;
    this.stats = { ticks: 0, rebalances: 0, swapsSent: 0, injectsDone: 0, skipped: 0, errors: 0 };
    this.state = 'idle';
  }
  ...
  getTickStats(): TickStats { return { ...this.stats }; }
  ...
  private async rebalanceViaSwap(
    ...
    const currentPrice = reservesToPrice(reserves.tokenReserve, reserves.wethReserve, ts.decimals);
  ...
  private getTokenRealAddress(ts: TokenSimState, _feedCache: FeedCache): string {
