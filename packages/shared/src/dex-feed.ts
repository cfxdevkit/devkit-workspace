...
const DEFAULT_CACHE_DIR = path.join(os.homedir(), ".devkit", "feeds");
...
async function gtFetch(urlPath: string, attempt = 0): Promise<unknown> {
	const MAX_RETRIES = 6;
	const resp = await fetch(`${GT_BASE}${urlPath}`, {
	...
	if (resp.status === 429 || resp.status === 503) {
		if (attempt >= MAX_RETRIES) {
			throw new Error(
				`GeckoTerminal ${urlPath} → HTTP ${resp.status} (exhausted ${MAX_RETRIES} retries)`,
			);
		}
	...
	if (!resp.ok) {
		throw new Error(`GeckoTerminal ${urlPath} → HTTP ${resp.status}`);
	}
	return resp.json();
}
...
async function fetchPoolsMulti(
	chain: string,
	poolAddresses: string[],
): Promise<GtPool[]> {
	if (poolAddresses.length === 0) return [];
	const data = (await gtFetch(
		`/networks/${chain}/pools/multi/${poolAddresses.join(",")}`,
	)) as {
		data: GtPool[];
	};
	return data.data ?? [];
...
function normalizeSelection(
	selection: TokenSourceSelection,
...
function buildSelectionKey(selections: TokenSourceSelection[]): string {
	return selections
		...
		.map(
			(selection) =>
				`${selection.poolAddress}:${selection.tokenAddress}:${selection.quoteMode ? "quote" : "base"}`,
		)
		.join("|");
}
...
function writeCache(cache: FeedCache, cacheDir: string): void {
	const filename = path.join(
		cacheDir,
		`${cache.chainId}-${cache.fetchedAt}.json`,
	);
...
async function selectTopTokens(
	...
	while (best.size < minCandidates && page <= maxPages) {
		...
		for (const pool of pools) {
			const baseAddr = normalizeAddress(
				pool.relationships.base_token.data.id,
			).toLowerCase();
			const quoteAddr = normalizeAddress(
			...
			if (isNativeToken(baseAddr)) {
				...
				if (!isStablecoin(quoteAddr) && !isNativeToken(quoteAddr)) {
					...
					const existingLiq = existing
						? parseFloat(existing.pool.attributes.reserve_in_usd) || 0
						: 0;
...
export async function fetchAndCache(
	...
	const { selections, wcfxPriceUsd } = await selectTopTokens(
		chain,
		cfg.tokenCount,
		cfg.skipStables,
		cfg.interReqMs,
...
export async function fetchAndCacheFromTokenSources(
	...
	const uniquePoolAddresses = [
		...new Set(normalizedSelections.map((selection) => selection.poolAddress)),
	];
	...
	for (let index = 0; index < uniquePoolAddresses.length; index += 20) {
		...
		for (const pool of pools) {
			poolsByAddress.set(
				normalizeAddress(pool.attributes.address).toLowerCase(),
				pool,
			);
		}
	}
	...
	for (const selection of normalizedSelections) {
		...
		if (!pool) {
			throw new Error(
				`Pool ${selection.poolAddress} was not returned by GeckoTerminal multi-pool snapshot.`,
			);
		}
...
export function isCacheValid(
	...
	const cache = JSON.parse(
		fs.readFileSync(files[files.length - 1], "utf-8"),
	) as FeedCache;
 ...
 */
export function listCacheVersions(
	chainId: number,
	cacheDir = DEFAULT_CACHE_DIR,
): string[] {
 ...
 */
export function exportCacheAsFixture(
	chainId: number,
	destPath: string,
	cacheDir = DEFAULT_CACHE_DIR,
): void {
	const files = listCacheFiles(chainId, cacheDir);
	if (!files.length)
		throw new Error(`No cache to export for chainId ${chainId}`);
