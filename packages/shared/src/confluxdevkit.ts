 ...
 */
export async function isDevkitServerRunning(
	config?: DevkitConfig,
): Promise<boolean> {
	try {
...
export async function generateMnemonicWords(
	...
	const res = await apiFetch<{ mnemonic: string }>(
		"/api/keystore/generate",
		config,
		{
			method: "POST",
			body: "{}",
 ...
 */
export async function restartWipeNode(
	config?: DevkitConfig,
): Promise<NodeStatus> {
	const res = await apiFetch<{ ok: boolean; status: NodeStatus }>(
...
export async function getNetworkCapabilities(
	config?: DevkitConfig,
...
export async function getContractTemplate(
	...
	return apiFetch<TemplateContract>(
		`/api/contracts/templates/${encodeURIComponent(name)}`,
		config,
	);
}
...
export async function getBootstrapEntry(
	name: string,
	config?: DevkitConfig,
): Promise<BootstrapEntry> {
...
export async function getFullStatus(config?: DevkitConfig): Promise<{
	...
	if (!serverOnline) {
		return {
			...
			nodeRunning: false,
			nextStep:
				'Start the conflux-devkit server: use "Conflux: Start DevKit Server" in VSCode, or run `npx conflux-devkit --no-open` in the terminal.',
		};
	}
	...
	try {
		keystoreStatus = await getKeystoreStatus(config);
	} catch {
		/* ignore */
	}
	try {
		nodeStatus = await getNodeStatus(config);
	} catch {
		/* ignore */
	}
	...
	if (!keystoreStatus?.initialized) {
		return {
			...
			nodeRunning: false,
			nextStep:
				"Keystore not initialized. Run `conflux_setup_init` to generate a mnemonic and complete first-time setup.",
		};
	}
	...
	if (keystoreStatus.locked) {
		return {
			...
			nodeRunning: false,
			nextStep:
				"Keystore is locked. Call unlockKeystore(password) to unlock it.",
		};
	}
	...
	if (!nodeRunning) {
		return {
			...
			nodeRunning: false,
			nextStep:
				"Server is ready. Call `conflux_node_start` to start the Conflux dev node.",
		};
	}
