...
export class TokenMirror {
   ...
   */
  recordMirror(
    token: Pick<TokenFeedData, 'realAddress' | 'symbol' | 'decimals' | 'iconCached'>,
    localAddress: string
  ): AddressTranslation {
    ...
    const idx = this.table.entries.findIndex(e => e.realAddress === entry.realAddress);
    if (idx >= 0) {
  ...
  isMirrorToken(address: string): boolean {
    return this.table.entries.some(e => e.localAddress === address.toLowerCase());
  }
  ...
  getEntry(realAddress: string): AddressTranslation | undefined {
    return this.table.entries.find(e => e.realAddress === realAddress.toLowerCase());
  }
}
