...
export class ContractsApplicationService {
	...
	async deploy(params: {
		...
		if (network.mode === "public") {
			...
			const resolvedRpcUrl =
				rpcUrl ??
				(chain === "evm"
					? network.public.evmRpcUrl
					: network.public.coreRpcUrl);
			if (!resolvedRpcUrl) {
				throw new Error(
					chain === "evm"
			...
			const resolvedChainId =
				chainId ??
				(chain === "evm"
					? effectiveChainIds.evmChainId
					: effectiveChainIds.chainId);
			...
			if (chain === "evm") {
				...
				if (!receiptAddress) {
					throw new Error(
						"Deploy timed out — receipt not found after 30 polls",
					);
				}
	...
	registerExternal(body: Partial<StoredContract>): StoredContract {
		const id =
			body.id ?? `${body.chain}-${body.address?.toLowerCase()}-${Date.now()}`;
	...
	async callContract(params: {
		...
		if (network.mode === "local") {
			...
			resolvedRpcUrl =
				contract.chain === "evm" ? localRpcUrls.evm : localRpcUrls.core;
			resolvedChainId =
				contract.chain === "evm" ? chainIds.evmChainId : chainIds.chainId;
		...
		if (contract.chain === "evm") {
			...
			const signerPrivateKey =
				network.mode === "local"
					? ((account?.evmPrivateKey ?? account?.privateKey) as
							| `0x${string}`
							| undefined)
					: (publicSigner?.privateKey as `0x${string}` | undefined);
			if (!signerPrivateKey) {
				throw new Error(
					"No signer private key available for write transaction",
				);
			}
