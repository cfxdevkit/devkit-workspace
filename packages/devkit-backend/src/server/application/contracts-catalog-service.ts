...
const TEMPLATES = {
  ...
  TestToken: {
    name: 'TestToken',
    description: 'ERC-20 token with mint/burn — useful for DeFi and transfer testing',
    source: TEST_TOKEN_SOURCE,
    get: getTestTokenContract,
  },
  Counter: {
    name: 'Counter',
    description: 'Ownable step counter with increment/decrement/reset — ideal first contract',
    source: COUNTER_SOURCE,
    get: getCounterContract,
  },
  BasicNFT: {
    name: 'BasicNFT',
    description: 'ERC-721 NFT from scratch — teaches token ownership, approvals, and transfers',
    source: ERC721_SOURCE,
    get: getERC721Contract,
  },
  Voting: {
    name: 'Voting',
    description: 'Ballot with vote delegation — teaches structs, weighted votes, governance',
    source: VOTING_SOURCE,
    get: getVotingContract,
  },
  Escrow: {
    name: 'Escrow',
    description: 'Three-party escrow with arbiter — teaches payable, state machines, CFX transfers',
    source: ESCROW_SOURCE,
    get: getEscrowContract,
  },
  MultiSigWallet: {
    name: 'MultiSigWallet',
    description: 'M-of-N multi-signature wallet — teaches collective governance and low-level call',
    source: MULTISIG_SOURCE,
    get: getMultiSigContract,
  },
  Registry: {
    name: 'Registry',
    description: 'On-chain name registry — teaches keccak256 keys, mappings, and string storage',
...
export class ContractsCatalogService {
  listTemplates(): Array<{ name: string; description: string; source: string }> {
  ...
  getTemplate(name: string): { name: string; source: string; abi: unknown[]; bytecode: string } | null {
