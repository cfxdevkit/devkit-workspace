...
const uiSharedCopyExcludes = new Set([
	"node_modules",
	"dist",
	".turbo",
	".next",
]);
const confluxWalletRoot = resolve(repoRoot, "packages", "conflux-wallet");
const confluxWalletCopyExcludes = new Set([
	"node_modules",
	"dist",
	".turbo",
	".next",
...
export function getTemplates() {
	return sortByName(
		listManifestDirectories(templatesRoot, "template.json").map((entry) => {
...
export function getTargets() {
	return sortByName(
		listManifestDirectories(targetsRoot, "target.json").map((entry) => {
...
function materializeConfluxWallet(destinationPath) {
	if (!existsSync(resolve(confluxWalletRoot, "package.json"))) {
		throw new Error(
			`Canonical conflux-wallet package not found: ${confluxWalletRoot}`,
		);
	}
	...
	copyDirectoryContents(
		...
		confluxWalletCopyExcludes,
	);

	const sourcePackage = readJson(resolve(confluxWalletRoot, "package.json"));
	const destinationPackage = readJson(
