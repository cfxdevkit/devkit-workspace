...
const copyExcludes = new Set(["node_modules", "dist", ".turbo", ".next"]);
...
function resolveAssetRoots() {
	const candidates = [
		{
			templatesRoot: resolve(packageRoot, "assets", "templates"),
			targetsRoot: resolve(packageRoot, "assets", "targets"),
			uiSharedRoot: resolve(packageRoot, "assets", "packages", "ui-shared"),
			confluxWalletRoot: resolve(
				packageRoot,
				"assets",
				"packages",
				"conflux-wallet",
		...
		{
			templatesRoot: resolve(workspaceRoot, "templates"),
			targetsRoot: resolve(workspaceRoot, "targets"),
			uiSharedRoot: resolve(workspaceRoot, "packages", "ui-shared"),
			confluxWalletRoot: resolve(workspaceRoot, "packages", "conflux-wallet"),
		},
	];

	for (const candidate of candidates) {
		if (
			directoryExists(candidate.templatesRoot) &&
			directoryExists(candidate.targetsRoot)
		) {
			return candidate;
		}
	}
	...
	throw new Error(
		"Scaffold assets were not found. Run pnpm --dir packages/scaffold-cli prepare-package before packing or publishing.",
	);
}

const assetRoots = resolveAssetRoots();
...
export function getTemplates() {
	return sortByName(
		listManifestDirectories(assetRoots.templatesRoot, "template.json").map(
			(entry) => {
...
export function getTargets() {
	return sortByName(
		listManifestDirectories(assetRoots.targetsRoot, "target.json").map(
			(entry) => {
...
export function resolveTemplateTarget(templateName, requestedTargetName) {
	...
	if (!targetName) {
		throw new Error(
			`Template ${template.name} does not declare a default target.`,
		);
	}
	...
	if (
		template.supportedTargets?.length &&
		!template.supportedTargets.includes(target.name)
	) {
		throw new Error(
			`Template ${template.name} does not support target ${target.name}`,
		);
	}
...
function copyDirectoryContents(
	...
	for (const entryName of readdirSync(sourcePath)) {
		...
		cpSync(join(sourcePath, entryName), join(destinationPath, entryName), {
			recursive: true,
		});
	}
}
...
function normalizePackageName(value) {
	const normalized = value
		...
		.toLowerCase()
		.replace(/[^a-z0-9-_]+/g, "-");
	return normalized.replace(/^-+|-+$/g, "") || "devkit-app";
}
...
function renderText(template, context) {
	return template
		...
		.replaceAll(
			"{{CODE_SERVER_ENABLED}}",
			String(context.targetFeatures.codeServer),
		);
}
...
function materializeUiShared(destinationPath) {
	if (!existsSync(resolve(assetRoots.uiSharedRoot, "package.json"))) {
		throw new Error(
			`Canonical ui-shared package not found: ${assetRoots.uiSharedRoot}`,
		);
	}

	copyDirectoryContents(
		assetRoots.uiSharedRoot,
		resolve(destinationPath, "ui-shared"),
		copyExcludes,
	);
...
function materializeConfluxWallet(destinationPath) {
	if (!existsSync(resolve(assetRoots.confluxWalletRoot, "package.json"))) {
		throw new Error(
			`Canonical conflux-wallet package not found: ${assetRoots.confluxWalletRoot}`,
		);
	}

	copyDirectoryContents(
		assetRoots.confluxWalletRoot,
		resolve(destinationPath, "conflux-wallet"),
		copyExcludes,
	);

	const sourcePackage = readJson(
		resolve(assetRoots.confluxWalletRoot, "package.json"),
	);
	const destinationPackage = readJson(
		resolve(destinationPath, "conflux-wallet", "package.json"),
	);
	if (sourcePackage.version !== destinationPackage.version) {
		throw new Error("conflux-wallet version mismatch after materialization");
	}
...
function writeTargetModule(destinationPath, template, payload) {
	const relativeTargetModulePath =
		template.generated?.targetModulePath ?? "src/generated/devkit-target.js";
	const absoluteTargetModulePath = resolve(
		destinationPath,
		relativeTargetModulePath,
	);
...
function writeGenerationManifest(destinationPath, payload) {
	const metadataDir = resolve(destinationPath, ".devkit");
	mkdirSync(metadataDir, { recursive: true });
	writeFileSync(
		resolve(metadataDir, "manifest.json"),
...
export function createProject({ destinationPath, templateName, targetName }) {
	...
	let confluxWallet = null;
	if (template.materialize?.confluxWallet) {
		confluxWallet = materializeConfluxWallet(destinationPath);
	}
	...
	const materializedPackages = [uiShared, confluxWallet].filter(Boolean);
...
export function cleanPackagedAssets() {
	rmSync(resolve(packageRoot, "assets"), { recursive: true, force: true });
}
