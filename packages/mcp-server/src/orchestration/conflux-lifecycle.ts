...
import { wipeLocalDexState } from '../features/dex/dex.js';
...
export async function handleConfluxLifecycleTool(params: {
  ...
  switch (name) {
    case 'conflux_server_start': {
      ...
      if (alreadyRunning) {
        return { content: [{ type: 'text', text: '✓ conflux-devkit server is already running. Call conflux_status to continue.' }] };
      }

      const port = (devkitCfg.port ?? 7748).toString();
      const child = spawn('npx', ['conflux-devkit', '--no-open', '--port', port], {
        detached: true,
        stdio: 'ignore',
      });
      child.unref();
      ...
      if (!ready) {
        return {
          content: [{ type: 'text', text: 'conflux-devkit server did not become ready within 30 s. Try starting it manually in the terminal: npx conflux-devkit --no-open' }],
          isError: true,
        };
      }

      return {
        content: [{ type: 'text', text: '✅ conflux-devkit server started and ready. Call conflux_status to continue the setup lifecycle.' }],
      };
    }
    ...
    case 'conflux_status': {
      ...
      if (full.nodeRunning && ns?.rpcUrls) {
        lines.push('', `Core RPC:   ${ns.rpcUrls.core}`, `eSpace RPC: ${ns.rpcUrls.evm}`);
      }
    ...
    case 'conflux_node_status': {
      ...
      if (!online) {
        return {
          content: [{ type: 'text', text: 'conflux-devkit server is not running. Use conflux_status for full lifecycle check.' }],
          isError: true,
        };
      }
      ...
      const mining = status.mining
        ...
        : '';
      return {
        content: [{ type: 'text', text: `Node status: ${status.server}\nAccounts: ${status.accounts}${rpc}${mining}` }],
      };
    }
    ...
    case 'conflux_node_start': {
      ...
      if (!online) {
        return {
          content: [{ type: 'text', text: 'conflux-devkit server is not running. Start it first ("Conflux: Start DevKit Server" in VSCode or `npx conflux-devkit --no-open`).' }],
          isError: true,
        };
      }
    ...
    case 'conflux_node_wipe': {
      ...
      await wipeLocalDexState();
      return {
        content: [{ type: 'text', text: 'Node stopped and data wiped. Call conflux_node_start to bring it back up fresh.' }],
      };
    }
