	...
	switch (name) {
			...
			if (!online) {
				return {
					content: [
						{
							type: "text",
							text: 'conflux-devkit server is not running. Start it first with "Conflux: Start DevKit Server" in VSCode or run `npx conflux-devkit --no-open` in the terminal.',
						},
					],
					isError: true,
				};
			...
			if (ks.initialized) {
				return {
					content: [
						{
							type: "text",
							text: `Keystore already initialized${ks.locked ? " but locked — call conflux_keystore_unlock" : ". You can call conflux_node_start."}`,
						},
					],
				};
			}

			// Always use the standard devkit test mnemonic for local development.
			const mnemonic =
				...
				content: [
					{
						...
						text: [
							...
							"",
							"Mnemonic (standard devkit test mnemonic — do NOT use on mainnet):",
							`   ${mnemonic}`,
							"",
		...
		case "conflux_keystore_status": {
			...
			if (!online) {
				return {
					content: [
						{ type: "text", text: "conflux-devkit server is not running." },
					],
					isError: true,
				};
			}
			...
			if (!ks.initialized) {
				lines.push(
					"",
					"Action needed: call conflux_setup_init to initialize the keystore.",
				);
			} else if (ks.locked) {
				lines.push(
					"",
					"Action needed: call conflux_keystore_unlock with your password.",
				);
			} else {
		...
		case "conflux_keystore_unlock": {
			...
			return {
				content: [
					{
						type: "text",
						text: "Keystore unlocked. You can now call conflux_node_start.",
					},
				],
			};
		}
			...
			if (!wallets.length) {
				return {
					content: [
						{
							type: "text",
							text: "No wallets found. Run conflux_setup_init first.",
						},
					],
				};
			}
