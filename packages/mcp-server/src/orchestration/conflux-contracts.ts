import type { AccountInfo, BootstrapEntry, DevkitConfig } from '@cfxdevkit/shared';
...
function buildBootstrapArgs(params: {
  ...
  missingArgs: Array<{ index: number; name: string; type: string; description?: string }>;
  extraArgs: number;
} {
  const { entry, providedArgs, defaultAddress } = params;
  const schema = entry.constructorArgs ?? [];
  const resolvedArgs: unknown[] = [];
  const missingArgs: Array<{ index: number; name: string; type: string; description?: string }> = [];
  ...
  const extraArgs = providedArgs.length > schema.length ? providedArgs.length - schema.length : 0;
  return { resolvedArgs, missingArgs, extraArgs };
}

async function getDefaultEvmAddress(client: DevkitClient, config: DevkitConfig, accountIndex: number): Promise<string> {
  try {
...
function getConstructorSchema(abi: unknown[]): Array<{ name: string; type: string }> {
  if (!Array.isArray(abi)) {
    return [];
  }

  const constructorEntry = abi.find((entry) =>
    typeof entry === 'object' &&
    entry !== null &&
    'type' in entry &&
...
> {
  const { client, devkitCfg, contractName, providedArgs } = params;
  ...
  const extraArgs = providedArgs.length > constructorSchema.length ? providedArgs.length - constructorSchema.length : 0;
...
> {
  ...
  if (entry.chains?.length && !entry.chains.includes(chain)) {
    ...
    if (chain === 'core' && entry.chains.includes('evm') && entry.abi && entry.bytecode) {
      warnings.push(
  ...
  const defaultAddress = await getDefaultEvmAddress(client, devkitCfg, accountIndex);
...
export async function handleConfluxContractsTool(params: {
  ...
  switch (name) {
    case 'conflux_templates': {
      ...
      const lines = templates.map((t) => `• ${t.name.padEnd(20)} — ${t.description}`);
    ...
    case 'conflux_deploy': {
      ...
      if (missingArgs.length > 0) {
        const previewArgs = constructorSchema.map((arg, index) => {
          ...
          return supplied !== undefined ? JSON.stringify(supplied) : `"<${arg.name}>"`;
        });
        return {
          content: [{
            type: 'text',
            text: [
              ...
              ...missingArgs.map((arg) => `   - args[${arg.index}] ${arg.name}:${arg.type}`),
              '',
      ...
      const contract = await client.deployContractTemplate(contractName, contractArgs, chain, accountIndex, devkitCfg);
      return {
        content: [{
          type: 'text',
          text: [
            ...
            `ID:      ${contract.id}`,
            '',
            'Use cfxdevkit_contract_call / cfxdevkit_contract_write to interact.',
          ].join('\n'),
        }],
      };
    }
    ...
    case 'conflux_deploy_prepare': {
      ...
      const { template, constructorSchema, resolvedArgs, missingArgs, extraArgs } = prep;
      const report = {
        ready: missingArgs.length === 0 && extraArgs === 0,
        name: contractName,
        abiAvailable: Array.isArray(template.abi),
        bytecodeAvailable: typeof template.bytecode === 'string' && template.bytecode.length > 0,
      ...
      return {
        content: [{
          type: 'text',
          text: [
            report.ready ? '✅ Template deployment inputs are complete.' : '❌ Template deployment inputs are incomplete.',
    ...
    case 'conflux_contracts': {
      ...
      if (!contracts.length) {
        return { content: [{ type: 'text', text: 'No contracts deployed yet.' }] };
      }
    ...
    case 'conflux_bootstrap_catalog': {
      ...
      if (!catalog.length) {
        return { content: [{ type: 'text', text: 'Bootstrap catalog empty or server offline.' }] };
      }
      ...
      for (const [cat, entries] of byCategory) {
        ...
        for (const e of entries) {
          const note = e.type === 'precompile' ? ' (precompile, no deploy)' : '';
          const chainList = e.chains?.length ? ` [chains: ${e.chains.join(', ')}]` : '';
          lines.push(`  • ${e.name.padEnd(22)} — ${e.description}${note}${chainList}`);
          if (e.constructorArgs?.length) {
            lines.push(`    Args: ${e.constructorArgs.map((arg) => `${arg.name}:${arg.type}`).join(', ')}`);
          }
        }
      }
      return { content: [{ type: 'text', text: lines.join('\n') }] };
    ...
    case 'conflux_bootstrap_deploy': {
      ...
      if (missingArgs.length > 0) {
        ...
        return {
          content: [{
            ...
            text: [
              `❌ Missing required constructor args for ${contractName}.`,
              ...missingArgs.map((arg) => `   - args[${arg.index}] ${arg.name}:${arg.type}${arg.description ? ` — ${arg.description}` : ''}`),
              '',
              'Retry with all required args. Suggested call skeleton:',
      ...
      let contract: Awaited<ReturnType<DevkitClient['deployBootstrapContract']>>;
      try {
        contract = await client.deployBootstrapContract(contractName, contractArgs, chain, accountIndex, devkitCfg);
      } catch (apiErr) {
        if (chain === 'core') {
    ...
    case 'conflux_bootstrap_deploy_multi': {
      ...
      const chains = Array.isArray(a.chains)
        ? (a.chains as unknown[]).filter((v): v is 'evm' | 'core' => v === 'evm' || v === 'core')
        : [];
      if (chains.length === 0) {
        return {
          content: [{
      ...
      const providedArgs = (rawArgs as unknown[] | undefined) ?? [];
      const chainArgsInput = (a.chainArgs as Record<string, unknown> | undefined) ?? {};
      const accountIndex = (a.accountIndex as number | undefined) ?? 0;
      const continueOnError = (a.continueOnError as boolean | undefined) ?? true;
      ...
      for (const chain of uniqueChains) {
        ...
        if (!prep.ok) {
          hasFailure = true;
          results.push({ chain, ok: false, stage: 'prepare', error: prep.text });
        ...
        try {
          if (chain === 'core') {
            try {
              const contract = await client.deployBootstrapContract(contractName, constructorArgs, chain, accountIndex, devkitCfg);
              results.push({
                chain,
                ok: true,
                address: contract.address,
              ...
              throw new Error('Core deploy failed and catalog entry has no ABI/bytecode fallback.');
            }
          }
    ...
    case 'conflux_bootstrap_entry': {
      ...
      const argLines = schema.length
        ? schema.map((arg, index) => {
          const fallback = argDefault(arg.placeholder, defaultAddress);
          const fallbackNote = fallback ? ` | default=${JSON.stringify(fallback)}` : ' | required';
    ...
    case 'conflux_bootstrap_prepare': {
      ...
      const { entry, resolvedArgs, missingArgs, extraArgs, defaultAddress } = prep;
      ...
      return {
        content: [{
          type: 'text',
          text: [
            ready ? '✅ Deployment inputs are complete.' : '❌ Deployment inputs are incomplete.',
            JSON.stringify(report, null, 2),
            '',
