	...
	switch (name) {
		...
		case "conflux_accounts": {
			...
			if (!accounts.length) {
				return {
					content: [
						{ type: "text", text: "No accounts found. Is the node running?" },
					],
				};
			}
		...
		case "conflux_mining_start": {
			...
			return {
				content: [
					{
						type: "text",
						text: `Auto-mining started (interval: ${intervalMs}ms). Running: ${status.isRunning}`,
					},
				],
			};
		}
		...
		case "conflux_mining_stop": {
			...
			return {
				content: [
					{
						type: "text",
						text: `Auto-mining stopped. Blocks mined: ${status.blocksMined ?? 0}`,
					},
				],
			};
		}
