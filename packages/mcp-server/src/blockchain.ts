 ...
 *  - eSpace write (3): sendCFX, writeContract, deployContract
 *  - Core read    (4): balance, block, chainId, callContract
 *  - Core write   (3): sendCFX, writeContract, deployContract
 ...
 */
function resolveKey(a: Record<string, unknown>, chain: 'espace' | 'core'): string {
  if (a.privateKey && typeof a.privateKey === 'string' && a.privateKey.startsWith('0x')) {
    return a.privateKey;
  }
...
export const blockchainToolDefinitions = [
  // ── Keystore / Account tools ──────────────────────────────────────────────
  {
    ...
    inputSchema: {
      type: 'object',
      properties: {
        includeIndex: { type: 'boolean', description: 'Show account index (default: true)' },
      },
    },
  },
  {
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        nameOrAddress: { type: 'string', description: 'Contract name (e.g. "CDK Token") or 0x address' },
      },
    },
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        nameOrAddress: { type: 'string', description: 'Contract name or address' },
        functionName: { type: 'string', description: 'View/pure function name' },
        args: { type: 'string', description: 'Flat JSON array of args. Core contract address args → "net2029:aa…". eSpace → "0x…". uint256 → decimal string.' },
        rpcUrl: { type: 'string' },
        chainId: { type: 'number' },
      },
    },
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        args: { type: 'string', description: 'Flat JSON array of args in parameter order. uint256 → decimal string.' },
        accountIndex: { type: 'number', description: 'Caller account index from keystore (default: 0)' },
        privateKey: { type: 'string', description: '0x-prefixed private key (overrides accountIndex)' },
        value: { type: 'string', description: 'CFX to send with call (payable functions, e.g. "1.0")' },
        rpcUrl: { type: 'string' },
        chainId: { type: 'number' },
      },
    },
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        contractId: { type: 'string', description: 'Contract id from /api/contracts/deployed (e.g. "evm-1713000000000") or contract address' },
        ...
        accountIndex: { type: 'number', description: 'Account index for write calls (default: 0)' },
        port: { type: 'number', description: 'Devkit server port (default: 7748)' },
      },
    },
  },
  // ── eSpace — read ─────────────────────────────────────────────────────────
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        chainId: { type: 'number', description: 'Chain ID (default: 2030 for local)' },
      },
    },
  },
  {
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        abi: { type: 'string', description: 'Contract ABI as JSON string or array' },
        functionName: { type: 'string', description: 'Name of the view/pure function to call' },
        args: {
          type: 'array',
          description:
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        tokenAddress: { type: 'string', description: 'ERC20 token contract address (0x...)' },
        holderAddress: { type: 'string', description: 'Address to check balance for (optional)' },
        spenderAddress: { type: 'string', description: 'Spender to check allowance for (optional, requires holderAddress)' },
        rpcUrl: { type: 'string' },
        chainId: { type: 'number' },
      },
    },
  },
  {
    ...
    description:
      'Get the transaction receipt for a submitted eSpace transaction. ' +
      'Returns status (success/reverted), gas used, block number, and logs. ' +
      'Polls until the tx is mined (up to 30s).',
    inputSchema: {
      type: 'object',
      required: ['txHash'],
      properties: {
  ...
  // ── eSpace — write ────────────────────────────────────────────────────────
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        accountIndex: { type: 'number', description: 'Sender account index from keystore (default: 0)' },
        privateKey: { type: 'string', description: '0x-prefixed private key (overrides accountIndex)' },
        rpcUrl: { type: 'string' },
        chainId: { type: 'number' },
      },
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        functionName: { type: 'string', description: 'Function name to call' },
        args: { type: 'string', description: 'Flat JSON array in parameter order, e.g. ["0xAddr","1000000000000000000"]. uint256 → decimal string. Never double-nest: ["val1","val2"] not ["["val1","val2"]"]' },
        value: { type: 'string', description: 'CFX to send with the call (payable, e.g. "1.0")' },
        accountIndex: { type: 'number', description: 'Caller account index from keystore (default: 0)' },
        privateKey: { type: 'string', description: '0x-prefixed private key (overrides accountIndex)' },
        rpcUrl: { type: 'string' },
        chainId: { type: 'number' },
  ...
  },
  {
    ...
    inputSchema: {
      type: 'object',
      properties: {
        abi: { type: 'string', description: 'Contract ABI as JSON string (required if not providing source)' },
        bytecode: { type: 'string', description: 'Compiled bytecode 0x-prefixed (required if not providing source)' },
        source: { type: 'string', description: 'Solidity source code — auto-compiles with solc 0.8.28 (paris EVM target). Use instead of abi+bytecode.' },
        constructorArgs: { type: 'string', description: 'JSON array of constructor arguments (default: [])' },
        contractName: { type: 'string', description: 'Contract name in source (for auto-compile) and/or human-readable name for tracking in .devkit-contracts.json' },
  ...
  // ── Core Space — read ─────────────────────────────────────────────────────
  {
    ...
    description:
      ...
      'Accepts Core Space base32 addresses (net2029:… / cfxtest:… / cfx:…) and hex addresses. ' +
      'Returns balance in CFX. Default RPC: http://localhost:12537.',
    inputSchema: {
      type: 'object',
      required: ['address'],
      properties: {
        address: { type: 'string', description: 'Core Space address (net2029:… / cfxtest:… / cfx:…) or hex' },
        rpcUrl: { type: 'string', description: 'Core Space RPC URL (default: http://localhost:12537)' },
        chainId: { type: 'number', description: 'Chain ID (default: 2029 for local)' },
      },
    },
  },
  {
  ...
  },
  {
    ...
    inputSchema: {
      ...
      required: ['address', 'abi', 'functionName'],
      properties: {
        address: { type: 'string', description: 'Core contract address (net2029:… / cfxtest:… / cfx:…)' },
        abi: { type: 'string', description: 'Contract ABI as JSON string' },
        functionName: { type: 'string', description: 'View/pure function to call' },
        args: { type: 'string', description: 'Flat JSON array in parameter order, e.g. ["0xAddr","1000000000000000000"]. uint256 → decimal string. Never double-nest.' },
        rpcUrl: { type: 'string' },
        chainId: { type: 'number' },
      },
    },
  ...
  // ── Core Space — write ────────────────────────────────────────────────────
  {
    ...
    inputSchema: {
      ...
      required: ['to', 'amount'],
      properties: {
        to: { type: 'string', description: 'Recipient: Core address or eSpace 0x address' },
        amount: { type: 'string', description: 'Amount in CFX (e.g. "100")' },
        accountIndex: { type: 'number', description: 'Sender account index from keystore (default: 0)' },
        privateKey: { type: 'string', description: '0x-prefixed Core Space private key (overrides accountIndex)' },
        rpcUrl: { type: 'string' },
        chainId: { type: 'number' },
      },
    },
  },
  {
    ...
    inputSchema: {
      ...
      required: ['address', 'abi', 'functionName'],
      properties: {
        address: { type: 'string', description: 'Core contract address' },
        abi: { type: 'string', description: 'Contract ABI as JSON string' },
        functionName: { type: 'string', description: 'Function name to call' },
        args: { type: 'string', description: 'Flat JSON array in parameter order, e.g. ["0xAddr","1000000000000000000"]. uint256 → decimal string. Never double-nest.' },
        value: { type: 'string', description: 'CFX value to send with call (payable, e.g. "1.0")' },
        accountIndex: { type: 'number', description: 'Caller account index from keystore (default: 0)' },
        privateKey: { type: 'string', description: '0x-prefixed Core Space private key (overrides accountIndex)' },
        rpcUrl: { type: 'string' },
        chainId: { type: 'number' },
  ...
  },
  {
    ...
    description:
      ...
      'Returns the deployed contract address and tracks it in .devkit-contracts.json. ' +
      'Specify the deployer by accountIndex (0-based, default 0) — key resolved from keystore. ' +
    ...
    inputSchema: {
      ...
      properties: {
        abi: { type: 'string', description: 'Contract ABI as JSON string (required if not providing source)' },
        bytecode: { type: 'string', description: 'Compiled bytecode 0x-prefixed (required if not providing source)' },
        source: { type: 'string', description: 'Solidity source — auto-compiles instead of abi+bytecode' },
        constructorArgs: { type: 'string', description: 'JSON array of constructor arguments (default: [])' },
        contractName: { type: 'string', description: 'Contract name in source and/or human-readable name for tracking' },
        accountIndex: { type: 'number', description: 'Deployer account index from keystore (default: 0)' },
        privateKey: { type: 'string', description: '0x-prefixed Core Space private key (overrides accountIndex)' },
        rpcUrl: { type: 'string' },
        chainId: { type: 'number' },
  ...
  // ── Wallet tools ──────────────────────────────────────────────────────────
  {
    ...
    inputSchema: {
      ...
      required: ['mnemonic'],
      properties: {
        mnemonic: { type: 'string', description: 'BIP-39 12 or 24 word mnemonic phrase' },
        count: { type: 'number', description: 'Number of accounts to derive (default: 5)' },
        startIndex: { type: 'number', description: 'Start index (default: 0)' },
        coreNetworkId: { type: 'number', description: 'Core Space network ID for address encoding (default: 2029 for local)' },
        includeFaucet: { type: 'boolean', description: 'Also derive the faucet/mining account (default: true)' },
      },
    },
  },
  {
    ...
    inputSchema: {
      ...
      required: ['mnemonic'],
      properties: {
        mnemonic: { type: 'string', description: 'Mnemonic phrase to validate' },
      },
    },
  },
  {
...
export async function blockchainToolHandler(
  ...
  const espaceChainId = (a.chainId as number | undefined) ?? DEFAULT_ESPACE_CHAIN_ID;
  const coreChainId   = (a.chainId as number | undefined) ?? DEFAULT_CORE_CHAIN_ID;
  ...
  switch (name) {
    ...
    case 'cfxdevkit_get_accounts': {
      ...
      for (const acc of accounts) {
        ...
        lines.push(`  Core address:   ${acc.coreAddress}   <- use for Core Space operations`);
        lines.push(`  eSpace address: ${acc.evmAddress}   <- use for eSpace (EVM) operations`);
      }
      lines.push('\nADDRESS FORMAT RULES:');
      lines.push('  Core Space operations  -> always pass the "Core address" (net2029:aa… / cfxtest:aa… / cfx:aa…)');
      lines.push('  eSpace operations      -> always pass the "eSpace address" (0x…)');
      lines.push('  These are DIFFERENT addresses — do NOT use a 0x address for Core or vice-versa.');
    ...
    case 'cfxdevkit_get_contract_info': {
      ...
      if (!contract) {
        const all = (await readContracts()).map(c => `  • ${c.name} (${c.address})`).join('\n');
        return {
      ...
      if (contract.abi) {
        try {
          const parsed = (contract.abi ?? []) as { type?: string; name?: string; stateMutability?: string }[];
          const fns = parsed.filter(e => e.type === 'function');
          const reads = fns.filter(e => e.stateMutability === 'view' || e.stateMutability === 'pure');
          const writes = fns.filter(e => e.stateMutability !== 'view' && e.stateMutability !== 'pure');
          abiFunctions = [
    ...
    case 'cfxdevkit_contract_call': {
      const contract = await findContract(a.nameOrAddress as string);
      if (!contract) {
        return { text: `Contract "${a.nameOrAddress}" not found. Run cfxdevkit_list_contracts to see tracked contracts.`, isError: true };
      }
      if (!contract.abi) {
        return { text: `Contract "${contract.name}" has no stored ABI. Use blockchain_espace_call_contract or blockchain_core_call_contract and provide the ABI manually.`, isError: true };
      }
      const rpc = (a.rpcUrl as string | undefined) ?? (contract.chain === 'evm' ? DEFAULT_ESPACE_RPC : DEFAULT_CORE_RPC);
      ...
      if (contract.chain === 'evm') {
        ...
        const pub = client.getInternalClient() as { readContract: (opts: unknown) => Promise<unknown> };
      ...
      const addressNote = contract.chain !== 'evm'
    ...
    case 'cfxdevkit_contract_write': {
      ...
      if (!contract.abi) {
        return { text: `Contract "${contract.name}" has no stored ABI. Use blockchain_espace_write_contract or blockchain_core_write_contract and provide the ABI manually.`, isError: true };
      }
      const rpc = (a.rpcUrl as string | undefined) ?? (contract.chain === 'evm' ? DEFAULT_ESPACE_RPC : DEFAULT_CORE_RPC);
      const chainId = (a.chainId as number | undefined) ?? contract.chainId;
      const abi = parseAbi(contract.abi);
      ...
      if (contract.chain === 'evm') {
        ...
        const valueCore = a.value ? BigInt(Math.floor(parseFloat(a.value as string) * 1e18)) : undefined;
        const hash = await wallet.writeContract(
          contract.address,
          abi,
          a.functionName as string,
    ...
    case 'cfxdevkit_api_contract_call': {
      ...
      if (contractIdOrAddr.startsWith('0x')) {
        ...
        if (!listRes.ok) {
          return { text: `Failed to fetch contracts from devkit API: ${listRes.statusText}`, isError: true };
        }
        const contracts = await listRes.json() as { id: string; address: string }[];
        const found = contracts.find(c => c.address.toLowerCase() === contractIdOrAddr.toLowerCase());
        if (!found) {
          return { text: `No contract with address ${contractIdOrAddr} found in devkit API. Use cfxdevkit_list_contracts for MCP-tracked contracts.`, isError: true };
        }
        contractId = found.id;
      }

      const callRes = await fetch(`${base}/api/contracts/${encodeURIComponent(contractId)}/call`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ functionName, args, accountIndex }),
      });

      const data = await callRes.json() as { success?: boolean; result?: unknown; txHash?: string; blockNumber?: string; status?: string; error?: string };

      if (!callRes.ok || !data.success) {
        return { text: `API call failed: ${data.error ?? callRes.statusText}`, isError: true };
      }
    ...
    case 'blockchain_espace_get_chain_id': {
      ...
      const known: Record<number, string> = { 2030: 'local devkit', 71: 'testnet', 1030: 'mainnet' };
      return {
    ...
    case 'blockchain_espace_read_erc20': {
      ...
      const [name, symbol, decimals, totalSupply] = await Promise.all([
        client.publicClient.readContract({ address: addr, abi: ERC20_ABI, functionName: 'name' }) as Promise<string>,
        client.publicClient.readContract({ address: addr, abi: ERC20_ABI, functionName: 'symbol' }) as Promise<string>,
        client.publicClient.readContract({ address: addr, abi: ERC20_ABI, functionName: 'decimals' }) as Promise<number>,
        client.publicClient.readContract({ address: addr, abi: ERC20_ABI, functionName: 'totalSupply' }) as Promise<bigint>,
      ]);
      const lines = [
        `Token:        ${name} (${symbol})`,
        `Address:      ${addr}`,
      ...
      if (a.holderAddress) {
        const balance = await client.publicClient.readContract({
          address: addr, abi: ERC20_ABI, functionName: 'balanceOf', args: [a.holderAddress as `0x${string}`],
        }) as bigint;
        ...
        if (a.spenderAddress) {
          const allowance = await client.publicClient.readContract({
            address: addr, abi: ERC20_ABI, functionName: 'allowance',
            args: [a.holderAddress as `0x${string}`, a.spenderAddress as `0x${string}`],
          }) as bigint;
          lines.push(`Allowance:    ${formatUnits(allowance, decimals)} ${symbol} (for ${a.spenderAddress})`);
        }
      }
      return { text: lines.join('\n') };
    }
    ...
    case 'blockchain_espace_deploy_contract': {
      ...
      const contractName = (a.contractName as string | undefined) ?? 'Unknown Contract';
      ...
      if (!abiRaw || !bytecodeRaw) {
        return { text: 'Error: provide either (abi + bytecode) or source for deployment.', isError: true };
      }
      ...
      try {
        await saveContract({
          ...
          chainId: espaceChainId,
          abi: typeof abiRaw === 'string' ? ((): unknown[] | undefined => { try { return JSON.parse(abiRaw) as unknown[]; } catch { return undefined; } })() : abiRaw as unknown[],
        });
      } catch { /* non-fatal */ }
      return {
        text: [
    ...
    case 'blockchain_core_get_balance': {
      ...
      const balance = await client.getBalance(a.address as Parameters<typeof client.getBalance>[0]);
    ...
    case 'blockchain_core_get_chain_id': {
      const client = coreClient(coreRpc, coreChainId);
      const chainId = await (client.getInternalClient() as { getChainId?: () => Promise<number> }).getChainId?.() ?? coreChainId;
      ...
      const net = known[chainId as number] ?? { name: 'custom', prefix: `net${chainId}` };
      return {
    ...
    case 'blockchain_core_write_contract': {
      ...
      const abi = parseAbi(a.abi);
      const args = parseArgs(a.args);
      const value = a.value ? BigInt(Math.floor(parseFloat(a.value as string) * 1e18)) : undefined;
      const hash = await wallet.writeContract(
        a.address as string,
        abi,
        a.functionName as string,
    ...
    case 'blockchain_derive_accounts': {
      ...
      const networkId = (a.coreNetworkId as number | undefined) ?? DEFAULT_CORE_CHAIN_ID;
      ...
      const accounts = deriveAccounts(mnemonic, { count, startIndex, coreNetworkId: networkId });
