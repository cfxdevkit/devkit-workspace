  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        source: {
          type: 'string',
          description: 'Full Solidity source code as a string. Must include pragma statement.',
        },
        contractName: {
          type: 'string',
          description:
        ...
        optimizer: {
          type: 'object',
          description: 'Optimizer settings. Default: { enabled: true, runs: 200 }',
          properties: {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        chain: {
          type: 'string',
          enum: ['evm', 'core'],
          description: 'Target chain — "evm" for eSpace (default), "core" for Core Space.',
        },
        accountIndex: {
          type: 'number',
          description: 'Keystore account index to deploy from (0-based, default 0).',
        },
        displayName: {
          type: 'string',
          description:
...
export async function compilerToolHandler(
  ...
  switch (name) {
    case 'cfxdevkit_list_templates': {
      const lines = Object.entries(TEMPLATES).map(([tname, t]) =>
        `• ${tname}\n  ${t.description}`
      );
    ...
    case 'cfxdevkit_get_template': {
      ...
      const fns = (abi as { type?: string; name?: string; stateMutability?: string }[])
        .filter(e => e.type === 'function')
        .map(e => `  ${e.stateMutability === 'view' || e.stateMutability === 'pure' ? '[read]' : '[write]'} ${e.name}()`);
    ...
    case 'cfxdevkit_compile_solidity': {
      ...
      const contractName = (args.contractName as string | undefined) ?? 'Contract';
      const optimizer = args.optimizer as { enabled: boolean; runs: number } | undefined;
      ...
      try {
        ...
        if (!result.success || result.contracts.length === 0) {
          const errors = result.errors.map(e => `  ${e.severity}: ${e.message}`).join('\n');
          return {
            text: [
              '✗ Compilation failed:',
        ...
        const fns = (contract.abi as { type?: string; name?: string; stateMutability?: string }[])
          .filter(e => e.type === 'function')
          .map(e => `  ${e.stateMutability === 'view' || e.stateMutability === 'pure' ? '[read]' : '[write]'} ${e.name}()`);
        ...
        return { text: `Compilation error: ${err instanceof Error ? err.message : String(err)}` };
      }
    }
    ...
    case 'cfxdevkit_compile_and_deploy': {
      ...
      const deployTool = chain === 'core' ? 'blockchain_core_deploy_contract' : 'blockchain_espace_deploy_contract';
      const displayName = (args.displayName as string | undefined) ?? contractName;
