...
const COMPOSE_CANDIDATES = ['docker-compose.yml', 'project-example/docker-compose.yml'];
...
function resolveComposeFile(cwd: string, explicitComposeFile?: string): {
  composeFile?: string;
  ...
  if (explicitComposeFile && explicitComposeFile.trim().length > 0) {
    return {
      ...
      source: 'argument',
      composeCandidates: COMPOSE_CANDIDATES.filter((candidate) => existsSync(join(cwd, candidate))),
    };
  }
  ...
  const composeCandidates = COMPOSE_CANDIDATES.filter((candidate) => existsSync(join(cwd, candidate)));
...
export function getWorkspaceContext(explicitComposeFile?: string): RuntimeContext {
  const cwd = process.cwd();
  const workspaceRoot = readNonEmptyEnv('CFXDEVKIT_AGENT_WORKSPACE') ?? cwd;
  const projectRoot = readNonEmptyEnv('CFXDEVKIT_PROJECT_ROOT') ?? workspaceRoot;
  const backendBaseUrl = readNonEmptyEnv('CFXDEVKIT_BACKEND_URL') ?? DEFAULT_BACKEND_BASE_URL;
  const runtimeMode = readNonEmptyEnv('CFXDEVKIT_RUNTIME_MODE') ?? inferRuntimeMode(cwd);
  ...
  return {
    ...
    source: compose.source === 'env' || readNonEmptyEnv('CFXDEVKIT_RUNTIME_MODE') || readNonEmptyEnv('CFXDEVKIT_AGENT_WORKSPACE')
      ? 'env'
      : compose.source,
  };
}

export function resolveDevkitPort(context: RuntimeContext, explicitPort?: number): number | undefined {
