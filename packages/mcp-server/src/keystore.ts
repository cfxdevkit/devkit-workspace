...
export function resolvePrivateKey(
  ...
  if (chain === 'espace') {
    if (!account.evmPrivateKey) throw new Error('Keystore encrypted — cannot resolve eSpace private key.');
    return account.evmPrivateKey;
  }
  if (!account.corePrivateKey) throw new Error('Keystore encrypted — cannot resolve Core private key.');
