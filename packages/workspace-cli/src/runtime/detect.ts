...
export function commandPath(cmd: string): string {
  try {
    const raw = execFileSync(LOOKUP_CMD, [cmd], { encoding: 'utf8', stdio: ['ignore', 'pipe', 'ignore'] }).trim();
    // `where.exe` on Windows may return multiple matches (one per line); take the first.
    return raw.split('\n')[0].trim();
  } catch {
    return '';
  }
...
export function detectSocket(runtime: Runtime, override: string | null): string | null {
  ...
  if (process.platform === 'win32') {
    // Docker Desktop and Podman Desktop expose the daemon via Windows named pipes.
    // The Docker CLI for Windows translates these into container volume mounts.
    return runtime === 'podman' ? '//./pipe/podman-desktop' : '//./pipe/docker_engine';
  }
