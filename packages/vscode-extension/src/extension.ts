...
export function activate(context: vscode.ExtensionContext): void {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(
			...
			async (networkId?: NetworkSelection) => {
				const id =
					networkId ??
					(await vscode.window
						.showQuickPick(
							[
								{
									label: "$(server-process) Local (dev)",
									description: "chainId 2029/2030",
									id: "local",
								},
								{
									label: "$(globe) Testnet",
									description: "chainId 1/71",
									id: "testnet",
								},
								{
									label: "$(globe) Mainnet",
									description: "chainId 1029/1030",
									id: "mainnet",
								},
							],
	...
	registerConfluxCommands(context, {
	...
	if (autoStart) {
		setTimeout(
			() => void vscode.commands.executeCommand("cfxdevkit.serverStart"),
			500,
		);
	}
}
