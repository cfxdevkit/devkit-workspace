    ...
    vscode.commands.registerCommand('cfxdevkit.initializeSetup', async () => {
      ...
      try {
        ...
        if (ks.initialized) {
          ...
          if (action === 'Unlock') vscode.commands.executeCommand('cfxdevkit.unlockKeystore');
          else if (action === 'Start Node') vscode.commands.executeCommand('cfxdevkit.nodeStart');
          return;
        }
          ...
          [
            {
              ...
              description: '12 BIP-39 words — creates a fresh development wallet',
        ...
        if (source.value === 'import') {
          ...
          try {
            mnemonic = await vscode.window.withProgress(
              { location: vscode.ProgressLocation.Notification, title: 'Generating mnemonic…', cancellable: false },
              () => generateMnemonic()
            );
          } catch (err) {
            vscode.window.showErrorMessage(`Failed to generate mnemonic: ${err instanceof Error ? err.message : String(err)}`);
            return;
          }
        }

        if (source.value === 'generate') {
          const channel = vscode.window.createOutputChannel('Conflux Wallet Setup');
          channel.clear();
          channel.appendLine('═══════════════════════════════════════════════');
        ...
        await vscode.window.withProgress(
          { location: vscode.ProgressLocation.Notification, title: 'Initializing keystore…', cancellable: false },
          () => setupKeystoreWallet(mnemonic, label || 'Default', { accountsCount })
        );
        ...
        vscode.window.showErrorMessage(`Setup failed: ${err instanceof Error ? err.message : String(err)}`);
      }
    })
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('cfxdevkit.unlockKeystore', async () => {
      ...
      try {
        ...
        vscode.window.showErrorMessage(`Unlock failed: ${err instanceof Error ? err.message : String(err)}`);
      }
    })
  );
}
