...
async function clearDexState(): Promise<void> {
  const dexPort = vscode.workspace.getConfiguration('cfxdevkit').get<number>('dexPort') ?? 8888;
  try {
...
export function registerNodeLifecycleCommands(params: {
    ...
    vscode.commands.registerCommand('cfxdevkit.nodeStart', async () => {
      ...
      try {
        const status = await vscode.window.withProgress(
          { location: vscode.ProgressLocation.Notification, title: 'Starting Conflux node…', cancellable: false },
          () => startNode()
        );
        ...
        vscode.window.showInformationMessage(
          ...
          if (a === 'View Accounts') vscode.commands.executeCommand('cfxdevkit.viewAccounts');
        });
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('cfxdevkit.nodeStop', async () => {
      ...
      try {
        await stopNode();
        vscode.window.showInformationMessage('Conflux node stopped. Blockchain data preserved.');
        providers?.accounts.clear();
        providers?.contracts.clear();
      } catch (err) {
        vscode.window.showErrorMessage(`Failed to stop node: ${err instanceof Error ? err.message : String(err)}`);
      }
    })
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('cfxdevkit.nodeRestart', async () => {
      ...
      try {
        const status = await vscode.window.withProgress(
          { location: vscode.ProgressLocation.Notification, title: 'Restarting Conflux node…', cancellable: false },
          () => restartNode()
        );
        vscode.window.showInformationMessage(`Conflux node restarted. Status: ${status.server}`);
        providers?.accounts.load();
        providers?.contracts.load();
      } catch (err) {
        const action = await vscode.window.showErrorMessage(
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('cfxdevkit.nodeWipeRestart', async () => {
      ...
      try {
        const status = await vscode.window.withProgress(
          { location: vscode.ProgressLocation.Notification, title: 'Wiping data and restarting node…', cancellable: false },
          () => restartWipe()
        );
        providers?.accounts.clear();
        providers?.contracts.clear();
        ...
        vscode.window.showErrorMessage(`Wipe-restart failed: ${err instanceof Error ? err.message : String(err)}`);
      }
    })
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('cfxdevkit.nodeWipe', async () => {
      ...
      try {
        ...
        providers?.contracts.clear();
        await clearDexState();
        vscode.window.showInformationMessage('Blockchain data wiped. Use "Conflux: Start Node" to restart.', 'Start Node')
          .then((a) => { if (a === 'Start Node') vscode.commands.executeCommand('cfxdevkit.nodeStart'); });
      } catch (err) {
        vscode.window.showErrorMessage(`Wipe failed: ${err instanceof Error ? err.message : String(err)}`);
      }
    })
  );
}
