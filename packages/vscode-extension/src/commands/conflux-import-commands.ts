  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('cfxdevkit.importContract', async () => {
      ...
      if (source.value === 'env') {
        ...
        if (!value) {
          vscode.window.showErrorMessage(`Environment variable ${envVar.trim()} is not set.`);
          return;
        }
        address = value.trim();
      } else {
      ...
      const chainDefault: 'evm' | 'core' = address.toLowerCase().startsWith('0x') ? 'evm' : 'core';
      ...
      const txHash = await vscode.window.showInputBox({
        title: 'Transaction Hash (optional)',
      ...
      try {
        ...
        vscode.window.showInformationMessage(`Imported ${name.trim()} on ${networkLabel}.`);
      } catch (err) {
        vscode.window.showErrorMessage(`Contract import failed: ${err instanceof Error ? err.message : String(err)}`);
      }
    })
  );
}
