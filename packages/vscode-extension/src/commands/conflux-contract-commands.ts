...
function buildArgsPlaceholder(fn: AbiFunction): string {
  ...
  const examples = fn.inputs.map((input) => {
    ...
    if (type.includes('uint') || type.includes('int')) return '"1000000000000000000"';
...
async function getSignerForWrite(functionName: string): Promise<{ accountIndex: number; privateKey?: string } | null> {
  ...
  if (capabilities && !capabilities.capabilities.contractWritePublic) {
    vscode.window.showErrorMessage('Public write is not available. Configure network RPC URLs first.');
    return null;
  }
  ...
  const source = await vscode.window.showQuickPick(
    ...
    { title: `Sign transaction for ${functionName}`, placeHolder: 'Public mode signer source' }
  );
...
export function registerContractCommands(params: {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('cfxdevkit.listContracts', async () => {
      ...
      try {
        ...
        if (!contracts.length) {
          vscode.window.showInformationMessage('No contracts deployed yet. Use "Conflux: Deploy Contract".');
          return;
        }
        const channel = vscode.window.createOutputChannel('Conflux Deployed Contracts');
        channel.clear();
        channel.appendLine('═══ Deployed Contracts ═══\n');
        for (const c of contracts) {
          channel.appendLine(`${(c.name ?? c.id).padEnd(24)} [${c.chain}]  ${c.address}`);
          if (c.deployedAt) channel.appendLine(`  Deployed: ${c.deployedAt}`);
        }
        channel.show(true);
      } catch (err) {
        vscode.window.showErrorMessage(`Failed to list contracts: ${err instanceof Error ? err.message : String(err)}`);
      }
    })
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      ...
      async (fn: AbiFunction, contract: LocalContract) => {
        const output = vscode.window.createOutputChannel('Conflux');
        ...
        if (fn.inputs.length > 0) {
          ...
          try {
            ...
            vscode.window.showErrorMessage('Invalid JSON — expected an array like ["arg1", "arg2"]');
            return;
          }
        }
        try {
          if (!contract.id) {
            vscode.window.showErrorMessage('Contract id missing; cannot call through devkit API.');
            return;
          }
          const result = await callDeployedContract(contract.id, fn.name, args, 0);
          output.appendLine(`[READ] ${contract.name ?? contract.address}.${fn.name}(${args.join(', ')})`);
          output.appendLine(`  → ${JSON.stringify(result.result ?? result)}`);
          output.show(true);
        } catch (err) {
          vscode.window.showErrorMessage(`Read failed: ${err instanceof Error ? err.message : String(err)}`);
        }
      }
    )
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      ...
      async (fn: AbiFunction, contract: LocalContract) => {
        ...
        if (!(await ensureBackendNetworkMode())) return;
        if (!contract.id) {
          vscode.window.showErrorMessage('Contract id missing; cannot send write transaction.');
          return;
        }
        ...
        try {
          const result = await vscode.window.withProgress(
            { location: vscode.ProgressLocation.Notification, title: `Sending ${fn.name}…`, cancellable: false },
            () => callDeployedContract(contract.id as string, fn.name, args, signer.accountIndex, signer.privateKey)
          );
          output.appendLine(`[WRITE] ${contract.name ?? contract.address}.${fn.name}(${JSON.stringify(args)})`);
          output.appendLine(`  → tx: ${result.txHash ?? 'unknown'}`);
          output.appendLine(`  → status: ${result.status ?? 'unknown'}`);
          output.show(true);
          if (result.txHash) {
            vscode.window.showInformationMessage(`✅ ${fn.name} → ${result.txHash.substring(0, 18)}…`);
          } else {
            vscode.window.showInformationMessage(`✅ ${fn.name} transaction sent`);
          }
        } catch (err) {
          vscode.window.showErrorMessage(`Write failed: ${err instanceof Error ? err.message : String(err)}`);
        }
      }
    )
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('cfxdevkit.copyContractAddress', async (item: { address?: string }) => {
