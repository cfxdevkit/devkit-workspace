...
import { workspaceUri, resolveMcpDist } from '../utils/fs';
...
function argDefault(placeholder: string | undefined, defaultAddr: string): string {
...
export function registerDeployCommands(params: {
  ...
  const { context, providers, requireServerOnline, ensureBackendNetworkMode } = params;
  ...
  async function getDeploySignerOptions(): Promise<{
    ...
    const source = await vscode.window.showQuickPick(
      [
        { label: 'Use keystore account index', id: 'index' as const, picked: true },
        { label: 'Override with private key', id: 'pk' as const },
      ],
      { title: 'Public deploy signer', placeHolder: 'Server env key override still has highest priority' }
    );
  ...
  async function deployFromBootstrap(): Promise<void> {
    ...
    try {
      ...
      vscode.window.showErrorMessage(`Failed to load catalog: ${err instanceof Error ? err.message : String(err)}`);
      return;
    }
    const deployable = catalog.filter((e) => e.type !== 'precompile');
    if (!deployable.length) {
      vscode.window.showErrorMessage('No deployable contracts in catalog. Is the node running?');
      return;
    }
    ...
    const chainPick = await vscode.window.showQuickPick(
      [
        { label: 'eSpace (EVM-compatible)', detail: 'chainId: 2030  ← recommended', id: 'evm' as const, picked: true },
        { label: 'Core Space', detail: 'chainId: 2029', id: 'core' as const },
      ],
      { title: `Deploy ${picked.label}`, placeHolder: 'Select target chain (default: eSpace)…' }
    );
    if (!chainPick) return;
    ...
    try {
      ...
      vscode.window.showErrorMessage(`Failed to load bootstrap artifact: ${err instanceof Error ? err.message : String(err)}`);
      return;
    }
    if (!entry.abi || !entry.bytecode) {
      vscode.window.showErrorMessage('Selected bootstrap contract has no deployable bytecode/ABI.');
      return;
    }
    ...
    try {
      const contract = await vscode.window.withProgress(
        { location: vscode.ProgressLocation.Notification, title: `Deploying ${picked.label}…`, cancellable: false },
        () => deployTemplate(picked.label, entry.abi as unknown[], entry.bytecode as string, args, chainPick.id, signer)
      );
      providers?.contracts.load();
      vscode.window.showInformationMessage(
        `✅ ${picked.label} deployed at ${contract.address}`,
        'List Contracts'
      ).then((a) => { if (a === 'List Contracts') vscode.commands.executeCommand('cfxdevkit.listContracts'); });
    } catch (err) {
      vscode.window.showErrorMessage(`Deployment failed: ${err instanceof Error ? err.message : String(err)}`);
    }
  }
  ...
  async function deployFromTemplate(): Promise<void> {
    ...
    try {
      ...
      vscode.window.showErrorMessage(`Failed to load templates: ${err instanceof Error ? err.message : String(err)}`);
      return;
    }
    if (!templates.length) {
      vscode.window.showErrorMessage('No templates available. Is the node running?');
      return;
    }

    const picked = await vscode.window.showQuickPick(
      templates.map((t) => ({ label: t.name, detail: t.description, source: t.source })),
    ...
    try {
      compiled = await vscode.window.withProgress(
        { location: vscode.ProgressLocation.Notification, title: `Compiling ${picked.label}…`, cancellable: false },
        () => compileContract(picked.source ?? '', picked.label)
      );
    } catch (err) {
      vscode.window.showErrorMessage(`Compilation failed: ${err instanceof Error ? err.message : String(err)}`);
      return;
    }
    ...
    const ctorAbi = (compiled.abi as Array<{ type?: string; inputs?: Array<{ name: string; type: string }> }>)
    ...
    try {
      const contract = await vscode.window.withProgress(
        { location: vscode.ProgressLocation.Notification, title: `Deploying ${picked.label}…`, cancellable: false },
        () => deployTemplate(picked.label, compiled.abi, compiled.bytecode, args, chainPick.id, signer)
      );
      providers?.contracts.load();
      vscode.window.showInformationMessage(
  ...
  async function deployDexStack(): Promise<void> {
    const dexAction = await vscode.window.showQuickPick(
      [
        { label: '🚀 Deploy V2 Stack', detail: 'Deploy Factory + WETH9 + Router02 to eSpace', id: 'deploy' as const },
        { label: '🌊 Seed Pools', detail: 'Import selected source pools from the known-token catalog and create local liquidity pairs', id: 'seed' as const },
        { label: '📦 Deploy + Seed (Full Setup)', detail: 'Deploy V2 stack then seed from the pool preset selection', id: 'full' as const },
      ],
    ...
    if (dexAction.id === 'deploy' || dexAction.id === 'full') {
      steps.push(`console.log('\\n🚀 Deploying V2 stack…');const d=await h('dex_deploy',{});console.log(d.text||d);`);
    }
    if (dexAction.id === 'seed' || dexAction.id === 'full') {
      const seedArgObj: Record<string, unknown> = {};
      if (selectedPoolAddresses && selectedPoolAddresses.length > 0) {
        seedArgObj.selectedPoolAddresses = selectedPoolAddresses;
      }
      if (selectedStablecoins && selectedStablecoins.length > 0) {
        seedArgObj.selectedStablecoins = selectedStablecoins;
      }
      const seedArgs = Object.keys(seedArgObj).length > 0 ? JSON.stringify(seedArgObj) : '{}';
      steps.push(`console.log('\\n🌊 Seeding pools from selected source pools…');const s=await h('dex_seed_from_gecko',${seedArgs});console.log(s.text||s);`);
    }
    steps.push(`console.log('\\n✅ Done.');`);

    const dexPath = await resolveMcpDist('dex', workspaceRoot);
    const script = [
      `import { dexToolHandler as h } from ${JSON.stringify(dexPath)};`,
      ...steps,
    ].join('\n');

    const tmpDir = os.tmpdir();
    const scriptFile = path.join(tmpDir, `dex-deploy-${Date.now()}.mjs`);
    fs.writeFileSync(scriptFile, script, 'utf-8');

    const terminal = vscode.window.createTerminal({ name: 'DEX Deploy', cwd: workspaceUri() });
    terminal.show();
    terminal.sendText(`node ${scriptFile}`);

    setTimeout(() => { providers?.contracts.load(); }, 15_000);
  }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('cfxdevkit.deployContract', async () => {
      ...
      const source = await vscode.window.showQuickPick(
        [
          { label: '📦 Bootstrap Catalog', detail: 'Production-ready contracts: ERC20, NFT, MultiSig, DeFi…', id: 'bootstrap' as const },
          { label: '🔧 Dev Template', detail: 'Simple dev contracts: Counter, SimpleStorage, TestToken…', id: 'template' as const },
          { label: '🔄 DEX (Uniswap V2)', detail: 'Deploy full V2 stack: Factory + WETH9 + Router + seed pools', id: 'dex' as const },
        ],
        { title: 'Conflux: Deploy Contract', placeHolder: 'Choose contract source…' }
      );
      if (!source) return;
