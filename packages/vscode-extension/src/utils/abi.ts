 ...
 */
export function encodeCalldata(fn: AbiFunction, args: unknown[]): Uint8Array | null {
  try {
 ...
 */
export function decodeResult(outputs: AbiInput[] | undefined, hex: string): string {
  ...
  try {
    ...
    for (const out of outputs) {
      if (out.type === 'uint256' || out.type.startsWith('uint')) {
        const val = BigInt(`0x${bytesToHex(bytes.slice(offset, offset + 32)).replace('0x', '')}`);
        results.push(val.toString());
        offset += 32;
      } else if (out.type === 'address') {
        results.push(`0x${bytesToHex(bytes.slice(offset + 12, offset + 32)).replace('0x', '')}`);
        offset += 32;
      } else if (out.type === 'bool') {
        results.push(bytes[offset + 31] !== 0);
        offset += 32;
      } else if (out.type === 'string') {
        const strOffset = Number(BigInt(`0x${bytesToHex(bytes.slice(offset, offset + 32)).replace('0x', '')}`));
        const strLen = Number(BigInt(`0x${bytesToHex(bytes.slice(strOffset, strOffset + 32)).replace('0x', '')}`));
        results.push(new TextDecoder().decode(bytes.slice(strOffset + 32, strOffset + 32 + strLen)));
        offset += 32;
      } else {
...
function encodeStatic(type: string, val: unknown): Uint8Array | null {
  ...
  if (type.startsWith('uint') || type.startsWith('int')) {
    ...
    if (num < 0n) {
      const bits = type.startsWith('int') && type !== 'int'
        ? parseInt(type.replace('int', '') || '256', 10) : 256;
...
function keccak256(data: Uint8Array): Uint8Array {
  ...
  const state: bigint[][] = Array.from({ length: 5 }, () => new Array(5).fill(0n));
  ...
  for (let block = 0; block < padded.length; block += rate) {
    ...
    for (let round = 0; round < 24; round++) {
      ...
      const D: bigint[] = C.map((_, i) => C[(i + 4) % 5] ^ rotl64(C[(i + 1) % 5], 1));
