...
export function initPersistentProcessManager(context: vscode.ExtensionContext): void {
  const storageUri = context.storageUri ?? context.globalStorageUri;
  processStoreDir = path.join(storageUri.fsPath, 'managed-processes');
  fs.mkdirSync(processStoreDir, { recursive: true });
}

export function getManagedProcessRecord(id: string): ManagedProcessRecord | null {
...
export async function startManagedProcess(spec: ManagedProcessSpec): Promise<ManagedProcessRecord> {
  ...
  const stdout = cp.execFileSync('/bin/bash', ['-lc', launchCommand], {
    encoding: 'utf8',
    env: { ...process.env },
  }).trim();
...
export async function openManagedProcessLogs(id: string, label?: string): Promise<void> {
  const record = readRecord(id);
