 ...
 */
export function decodeConfluxAddress(address: string): {
	prefix: string;
	hexAddress: string;
} {
	...
	if (colonIdx < 0)
		throw new Error(`Invalid Conflux address (no colon): ${address}`);
	...
	const values = [...payload].map((c) => {
		...
		if (v === undefined)
			throw new Error(`Invalid base32 char '${c}' in address`);
		return v;
	});
 ...
 */
export function encodeConfluxAddress(
	hexAddress: string,
	networkPrefix: string,
): string {
	const clean = hexAddress.replace(/^0x/i, "");
	if (clean.length !== 40)
		throw new Error(`Expected 20-byte address, got: ${hexAddress}`);
 ...
 */
export function convertCoreAddressNetwork(
	coreAddress: string,
