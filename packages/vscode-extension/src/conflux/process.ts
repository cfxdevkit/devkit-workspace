...
/** Stops the active log tailer — called on deactivate or when tailer is replaced. */
let _stopLogTail: (() => void) | null = null;
...
export async function startDevkitProcess(): Promise<void> {
	if (await isServerOnline()) {
		...
		channel.appendLine(
			"[conflux-devkit] Server is already healthy (container-managed or previously started).",
		);
		return;
	}
	...
	// Stream the managed-process log into the output channel so users see live output.
	_stopLogTail?.();
	_stopLogTail = tailManagedProcessLog(SERVER_PROCESS_ID, channel);
	...
	channel.appendLine(
		"[conflux-devkit] Server did not respond before timeout. Check logs if startup failed.",
	);
}
...
export async function stopDevkitProcess(): Promise<void> {
	...
	if (await isServerOnline()) {
		throw new Error(
			"Conflux devkit backend is running as a container-managed service and cannot be stopped from the extension.",
		);
	}
...
export async function showDevkitProcessLogs(): Promise<void> {
	...
	const terminal = vscode.window.createTerminal({
		...
		shellArgs: [
			"-lc",
			`if [ -f '${CONTAINER_BACKEND_LOG}' ]; then tail -n 200 -f '${CONTAINER_BACKEND_LOG}'; else echo 'No backend log found at ${CONTAINER_BACKEND_LOG}'; fi`,
		],
	});
	terminal.show(true);
}
