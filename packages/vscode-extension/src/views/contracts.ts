...
import { getDeployedContracts, type DeployedContract } from '../conflux/api';
import type { AbiFunction } from '../conflux/rpc';
import { networkState, type NetworkSelection } from './network-state';
...
class NetworkGroupItem extends vscode.TreeItem {
  constructor(
    ...
    this.description = isActiveNetwork ? 'active' : `${bucket.contracts.length} contract(s)`;
...
class ContractItem extends vscode.TreeItem {
  constructor(readonly contract: LocalContract) {
    ...
    this.tooltip = [
      ...
      contract.deployedAt ? `Deployed: ${new Date(contract.deployedAt).toLocaleString()}` : '',
  ...
  parsedAbi(): AbiFunction[] {
    ...
    try {
      return (JSON.parse(this.contract.abi) as AbiFunction[])
...
class AbiFunctionItem extends vscode.TreeItem {
  constructor(
    ...
    const sigArgs = fn.inputs.map(i => `${i.type} ${i.name || ''}`.trim()).join(', ');
    ...
    this.iconPath = new vscode.ThemeIcon(isRead ? 'symbol-property' : 'symbol-event');
  }
}

// ── Provider ──────────────────────────────────────────────────────────────────

export class ContractsProvider implements vscode.TreeDataProvider<vscode.TreeItem>, vscode.Disposable {
  ...
  getChildren(element?: vscode.TreeItem): vscode.ProviderResult<vscode.TreeItem[]> {
    if (!element) {
      ...
      return buckets.map((bucket) => new NetworkGroupItem(bucket, bucket.key === activeKey));
    }

    if (element instanceof NetworkGroupItem) {
      const evmContracts = element.bucket.contracts.filter(c => c.chain === 'evm');
      const coreContracts = element.bucket.contracts.filter(c => c.chain === 'core');
      const groups: ChainGroupItem[] = [];
      if (evmContracts.length > 0) groups.push(new ChainGroupItem('eSpace', evmContracts));
      if (coreContracts.length > 0) groups.push(new ChainGroupItem('Core Space', coreContracts));
      return groups;
    }
    ...
    if (element instanceof ContractItem) {
      ...
      const items: vscode.TreeItem[] = [new ContractAddressItem(element.contract)];
      ...
      if (reads.length) items.push(new AbiGroupItem('READ', reads, element.contract));
      if (writes.length) items.push(new AbiGroupItem('WRITE', writes, element.contract));
      return items;
    }
...
function buildNetworkBuckets(contracts: LocalContract[]): NetworkBucket[] {
  ...
  const preferredOrder: Record<string, number> = { local: 0, testnet: 1, mainnet: 2 };
