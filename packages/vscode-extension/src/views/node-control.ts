...
export class NodeControlProvider implements vscode.TreeDataProvider<vscode.TreeItem> {
  ...
  getChildren(element?: vscode.TreeItem): vscode.ProviderResult<vscode.TreeItem[]> {
    ...
    if (networkState.selected !== 'local') {
      ...
      info.command = { command: 'cfxdevkit.selectNetwork', title: 'Select Network', arguments: ['local'] };
      return [info];
    }
    ...
    const stoppedAction = this.localState === 'stopped' ? 'cfxdevkit.nodeStoppedChoice' : undefined;
    items.push(new NodeStatusItem(text, icon, colorId, stoppedAction));
    items.push(new NodeDividerItem());

    switch (this.localState) {
        case 'offline':
          items.push(new NodeActionItem(
            'Start DevKit', 'server-process', 'cfxdevkit.serverStart',
        ...
        case 'setup-req':
          items.push(new NodeActionItem(
            'Init Wallet', 'key', 'cfxdevkit.initializeSetup',
        ...
        case 'locked':
          items.push(new NodeActionItem(
            'Unlock', 'lock', 'cfxdevkit.unlockKeystore',
        ...
        default: {
          ...
          if (this.localState !== 'running' && this.localState !== 'starting') {
            items.push(new NodeActionItem('Start Node', 'play', 'cfxdevkit.nodeStart'));
          }
          if (this.localState === 'running' || this.localState === 'starting') {
            items.push(new NodeActionItem('Stop Node', 'debug-stop', 'cfxdevkit.nodeStop'));
            items.push(new NodeActionItem('Restart Node', 'debug-restart', 'cfxdevkit.nodeRestart'));
          }
          items.push(new NodeActionItem(
            'Wipe & Restart', 'warning', 'cfxdevkit.nodeWipeRestart',
  ...
  private statusDisplay(): { text: string; icon: string; colorId?: string } {
    switch (this.localState) {
      ...
      case 'setup-req':
        return { text: 'Wallet setup required', icon: 'key',
          colorId: 'notificationsWarningIcon.foreground' };
      case 'locked':
        return { text: 'Keystore locked', icon: 'lock',
          colorId: 'notificationsWarningIcon.foreground' };
      case 'stopped':
        return { text: 'Node stopped', icon: 'debug-stop',
          colorId: 'testing.iconFailed' };
      case 'starting':
        return { text: 'Node starting…', icon: 'loading~spin' };
      case 'running':
        return {
          text: this.accountCount > 0 ? `Running  (${this.accountCount} accounts)` : 'Running',
          icon: 'flame',
