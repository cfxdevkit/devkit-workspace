...
async function getTokenAddress(pairAddress: string, selector: '0x0dfe1681' | '0xd21220a7'): Promise<string | null> {
  try {
...
async function getReserves(pairAddress: string): Promise<{ reserve0: bigint; reserve1: bigint } | null> {
  try {
...
async function getPair(factoryAddress: string, tokenA: string, tokenB: string): Promise<string | null> {
  try {
...
async function readManifest(): Promise<V2Manifest | null> {
  try {
    const r = await fetch(`${getDexUrl()}/api/dex/manifest`, { signal: AbortSignal.timeout(3_000) });
    if (!r.ok) return null;
...
async function readTranslationTable(): Promise<TranslationTable | null> {
  try {
    const r = await fetch(`${getDexUrl()}/api/dex/translation-table`, { signal: AbortSignal.timeout(3_000) });
    if (!r.ok) return null;
...
class PoolItem extends vscode.TreeItem {
  constructor(readonly pool: PoolData) {
    ...
    const desc = usdStr ? `${wcfxStr} ${pool.quoteSymbol} (${usdStr})` : `${wcfxStr} ${pool.quoteSymbol}`;
    super(`${pool.symbol}/${pool.quoteSymbol}`, vscode.TreeItemCollapsibleState.Collapsed);
...
export class DexPoolsProvider implements vscode.TreeDataProvider<DexTreeItem>, vscode.Disposable {
  private _onDidChangeTreeData = new vscode.EventEmitter<DexTreeItem | undefined>();
  ...
  async load(): Promise<void> {
    ...
    if (table && table.entries.length > 0) {
      for (const entry of table.entries) {
        ...
        const isToken0 = entry.localAddress.toLowerCase() < this.manifest.contracts.weth9.toLowerCase();
        const tokenReserve = reserves ? (isToken0 ? reserves.reserve0 : reserves.reserve1) : 0n;
        const wcfxReserve  = reserves ? (isToken0 ? reserves.reserve1 : reserves.reserve0) : 0n;
      ...
      const pairs = deployed.filter(c => c.chain === 'evm' && /UniswapV2Pair/i.test(c.name ?? c.id ?? ''));
  ...
  getChildren(element?: DexTreeItem): DexTreeItem[] {
    ...
    if (element instanceof PoolItem) {
      ...
      return [
        ...
        new PoolDetailItem(`${p.symbol} Address`, p.localAddress, p.localAddress),
        ...(p.quoteAddress ? [new PoolDetailItem(`${p.quoteSymbol} Address`, p.quoteAddress, p.quoteAddress)] : []),
        new PoolDetailItem(`Reserve (${p.symbol})`, `${r0Human} ${p.symbol}`),
        new PoolDetailItem(`Reserve (${p.quoteSymbol})`, `${r1Human} ${p.quoteSymbol}`),
        new PoolDetailItem('Price', `${wcfxStr} ${p.quoteSymbol}/${p.symbol}${usdStr}`),
      ];
    }
