...
import { networkState, NETWORK_CONFIGS, type NetworkSelection } from './network-state';
...
class NetworkItem extends vscode.TreeItem {
  constructor(
    ...
    super(isSelected ? `${cfg.label}` : `${cfg.label}`, vscode.TreeItemCollapsibleState.None);
    let iconName = 'circle-large-outline';
    if (networkId === 'local') iconName = isSelected ? 'server-environment' : 'server';
