...
import { readKeystoreAccounts, type OfflineAccount } from '../conflux/keystore-reader';
...
class AccountGroupItem extends vscode.TreeItem {
  constructor(readonly ea: EnrichedAccount, readonly prefix: string) {
    ...
    this.iconPath = new vscode.ThemeIcon('account', new vscode.ThemeColor(balanceStateToColor(overallAccountState(ea))));
  }
}
...
class AddressItem extends vscode.TreeItem {
  constructor(
    ...
    super(chain === 'eSpace' ? 'eSpace' : `Core  (${networkPrefix})`, vscode.TreeItemCollapsibleState.None);
...
export class AccountsProvider implements vscode.TreeDataProvider<vscode.TreeItem> {
  ...
  private async loadPublicBalances(accounts: EnrichedAccount[], _network: NetworkSelection): Promise<void> {
  ...
  getChildren(element?: vscode.TreeItem): vscode.ProviderResult<vscode.TreeItem[]> {
    if (!element) {
      ...
      if (this.accounts.length === 0) {
        ...
        action.command = { command: 'cfxdevkit.initializeSetup', title: 'Initialize Wallet' };
        return [msg, action];
      }
    ...
    if (element instanceof AccountGroupItem) {
      ...
      return [
        new AddressItem(
          element.ea.evmAddress, 'eSpace', prefix,
          element.ea.evmBalance, element.ea.evmBalanceState ?? 'unknown',
        ),
        new AddressItem(
          element.ea.coreAddressForNetwork, 'Core', prefix,
          element.ea.coreBalance, element.ea.coreBalanceState ?? 'unknown',
        ),
      ];
    }
...
function formatBalanceShort(balance: string | undefined, state: BalanceState | undefined): string {
...
async function fetchEvmBalance(rpcUrl: string, address: string): Promise<BalanceResult> {
  try {
    const result = await jsonRpc<string>(rpcUrl, 'eth_getBalance', [address, 'latest']);
...
async function fetchCoreBalance(rpcUrl: string, address: string): Promise<BalanceResult> {
  try {
    const result = await jsonRpc<string>(rpcUrl, 'cfx_getBalance', [address, 'latest_state']);
    if (typeof result !== 'string') return { state: 'error' };
