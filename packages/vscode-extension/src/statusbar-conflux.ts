...
import { getCurrentNetwork, getKeystoreStatus, getNodeStatus, isServerOnline } from './conflux/api';
import { isServerProcessRunning, showDevkitProcessLogs } from './conflux/process';
...
export function registerConfluxStatusBar(
  ...
  const bar = vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Left, 9);
  bar.text = '$(loading~spin) CFX: –';

  function isEnabled(): boolean {
    return vscode.workspace.getConfiguration('cfxdevkit').get<boolean>('enabled', true);
  }

  function applyVisibility(): void {
    if (isEnabled()) bar.show(); else bar.hide();
  }
  ...
  context.subscriptions.push(
    networkState.onDidChange(() => void refresh())
  );
    ...
    try {
      ...
      if (!online) {
        if (isServerProcessRunning()) {
          ...
          bar.backgroundColor = undefined;
          bar.tooltip = 'Conflux devkit server process is running but the API is not ready yet. Click for logs or stop.';
        ...
        bar.backgroundColor = undefined;
        bar.tooltip = 'Conflux devkit server is offline.\nClick to start the DevKit server.';
      ...
      try {
        ...
        if (!ks.initialized) {
          ...
          bar.tooltip = 'Keystore not initialized. Click to run the wallet setup wizard.';
      ...
      switch (status.server) {
        ...
        case 'stopped':
          ...
          bar.backgroundColor = new vscode.ThemeColor('statusBarItem.warningBackground');
          bar.tooltip = 'Conflux node is stopped. Click to choose: Start Node or Switch Network.';
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('cfxdevkit.nodeStoppedChoice', async () => {
      const pick = await vscode.window.showQuickPick(
        ...
        { placeHolder: 'Conflux node is stopped' },
      );
      if (!pick) return;
      if (pick.id === 'start') {
      ...
      await vscode.commands.executeCommand('cfxdevkit.selectNetwork');
    })
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('cfxdevkit.confluxBarClick', async () => {
      const pick = await vscode.window.showQuickPick(
        [
          { label: '$(globe) Switch Network',                               id: 'network'         },
          { label: '$(output) View DevKit server logs',                       id: 'logs'            },
          { label: '$(person) View Accounts',                                  id: 'accounts'        },
          { label: '$(database) Mine 1 Block',                                 id: 'mine'            },
          { label: '$(debug-restart) Restart Node',                            id: 'restart'         },
          { label: '$(debug-stop) Stop Node',                                  id: 'stop'            },
        ],
        { placeHolder: 'Conflux local dev node' },
      );
      if (!pick) return;
      if (pick.id === 'network') { vscode.commands.executeCommand('cfxdevkit.selectNetwork'); return; }
      if (pick.id === 'logs')   { void showDevkitProcessLogs(); return; }
      ...
      vscode.commands.executeCommand(cmds[pick.id]);
    })
  );

  void refresh();
  const timer = setInterval(() => { if (isEnabled()) void refresh(); }, POLL_INTERVAL_MS);
