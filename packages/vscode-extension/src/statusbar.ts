...
type ProjectState = 'setup-needed' | 'idle' | 'dev' | 'prod' | 'dev+prod' | 'degraded';
...
function runScriptInTerminal(label: string, cwd: string, scriptName: string): void {
  const term = vscode.window.createTerminal({ name: label, cwd });
  term.show();
  term.sendText(scriptRunnerCommand(scriptName));
}

function runScriptWithEnvInTerminal(label: string, cwd: string, scriptName: string, env: Record<string, string>): void {
  const term = vscode.window.createTerminal({ name: label, cwd });
  term.show();
...
async function waitForServiceReady(
  ...
  return vscode.window.withProgress(
    ...
    async (progress) => {
      ...
      progress.report({ message: `Launch command sent. Waiting for port ${port}...` });
      ...
      progress.report({ increment: 100, message: 'Timed out waiting for the service.' });
      return false;
    },
  );
}
...
function getSelectedEspaceRpc(): string {
  if (networkState.selected === 'local') {
    return vscode.workspace.getConfiguration('cfxdevkit').get<string>('espaceRpc')
...
function getContractsArtifactPath(cwd: string): string {
  const artifactRel = cfg<string>('contractsArtifactPath', '.generated/project-example/dapp/src/generated/contracts-addresses.ts');
...
function getFrontendContractFilePath(cwd: string): string {
  const rel = '.generated/project-example/dapp/src/components/ExampleContract.tsx';
...
function resolvePathCandidates(cwd: string, configuredRel: string): string[] {
  ...
  if (normalized.startsWith('.generated/project-example/')) {
    candidates.add(path.join(cwd, normalized.replace(/^\.generated\/project-example\//, '')));
  }

  // If workspace is monorepo root and config is generated-project-relative missing, prefix it.
  if (!normalized.startsWith('.generated/project-example/') && fs.existsSync(path.join(cwd, '.generated', 'project-example'))) {
  ...
  candidates.add(path.join(cwd, '.generated/project-example/dapp/src/generated/contracts-addresses.ts'));

  return Array.from(candidates);
}

async function ethGetCode(rpcUrl: string, address: string): Promise<string | null> {
  try {
    const res = await fetch(rpcUrl, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
function extractAddressesForChain(tsSource: string, chainId: number): string[] {
  const block = new RegExp(`['"]?${chainId}['"]?\\s*:\\s*\\{([\\s\\S]*?)\\}`, 'm').exec(tsSource)?.[1] ?? '';
  if (!block) return [];
...
async function runProjectDoctor(cwd: string): Promise<DoctorReport> {
  ...
  const scriptProdRebuild = cfg<string>('scriptProdRebuild', 'stack:rebuild').trim();
  const scriptProdStatus = cfg<string>('scriptProdStatus', 'stack:status').trim();
  const scriptProdLogs = cfg<string>('scriptProdLogs', '').trim();

  checks.push(
    hasNodeModules(cwd)
      ? { key: 'deps.installed', level: 'ok', message: 'Dependencies: node_modules present' }
      : { key: 'deps.installed', level: 'warn', message: 'Dependencies missing: run install script' }
  );
    ...
    try {
      ...
      if (src.includes('/api/contracts/deployed')) {
        checks.push({
          ...
          level: 'warn',
          message: 'Frontend currently fetches /api/contracts/deployed at runtime; migrate to generated chainId/address artifact lookup',
        });
      } else {
        checks.push({
          key: 'frontend.addressSource',
          level: 'ok',
          message: 'Frontend does not rely on /api/contracts/deployed runtime lookup',
        });
      }
    } catch {
      checks.push({
  ...
  if (network === 'local') {
    ...
    if (!fs.existsSync(artifactAbs)) {
      ...
      try {
        ...
        const addresses = extractAddressesForChain(src, networkCfg.espaceChainId).slice(0, 5);
        if (!addresses.length) {
          checks.push({
            key: 'contracts.reachability',
            level: 'warn',
...
function renderDoctorSummary(report: DoctorReport): string {
  if (report.errorCount > 0) return `${report.errorCount} error(s), ${report.warnCount} warning(s)`;
...
export function registerStatusBar(context: vscode.ExtensionContext): void {
  ...
  async function requireScript(cwd: string, key: string, label: string, def = ''): Promise<string | null> {
    const scripts = readPackageScripts(cwd);
    ...
    if (!script) {
      ...
      if (act === 'Open Setting') {
        await vscode.commands.executeCommand('workbench.action.openSettings', `devkit.${key}`);
      }
      return null;
    }
    if (!hasScript(scripts, script)) {
      ...
      if (act === 'Open package.json') {
        await vscode.window.showTextDocument(vscode.Uri.file(path.join(cwd, 'package.json')));
      }
      if (act === 'Open Setting') {
        await vscode.commands.executeCommand('workbench.action.openSettings', `devkit.${key}`);
      }
  ...
  async function refresh(): Promise<void> {
    ...
    if (!hasNodeModules(cwd)) {
      ...
      bar.tooltip = 'Dependencies not installed. Click for the Project menu and run Install dependencies.';
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('devkit.projectDoctor', async () => {
      ...
      const configuredDoctor = await requireScript(cwd, 'scriptDoctor', 'Doctor', 'doctor');
      if (configuredDoctor) {
    ...
    vscode.commands.registerCommand('devkit.statusBarClick', async () => {
      ...
      const prodRunning = await isProdServerRunning(cfg<number>('prodPort', 3030));
      ...
      if (depsInstalled) {
        if (isLocalNetwork && !nodeRunning) {
          ...
          picks.push({ label: '$(play) Start dev environment', id: 'devStart' });
        } else {
          picks.push({ label: `$(link-external) Open dev app (port ${devPort})`, id: 'devOpen' });
          picks.push({ label: '$(output) View dev logs', id: 'devLogs' });
          picks.push({ label: '$(debug-stop) Stop dev environment', id: 'devStop' });
        }
      }
      ...
      if (!prodRunning) {
        picks.push({ label: '$(play) Start production preview', id: 'prodStart' });
        picks.push({ label: '$(refresh) Rebuild production preview', id: 'prodRebuild' });
      } else {
        picks.push({ label: `$(link-external) Open production app (port ${prodPort})`, id: 'prodOpen' });
        picks.push({ label: '$(output) Production logs', id: 'prodLogs' });
        picks.push({ label: '$(info) Production status', id: 'prodStatus' });
        picks.push({ label: '$(debug-stop) Stop production preview', id: 'prodStop' });
      }
      ...
      if (pick.id === 'install') {
        const script = await requireScript(cwd, 'scriptInstall', 'Install dependencies');
        if (!script) return;
      ...
      if (pick.id === 'clean') {
        const script = await requireScript(cwd, 'scriptClean', 'Clean workspace', 'clean');
        if (!script) return;
      ...
      if (pick.id === 'devOpen') {
        vscode.env.openExternal(
          await vscode.env.asExternalUri(vscode.Uri.parse(`http://127.0.0.1:${devPort}/`))
        );
        return;
      }
      ...
      if (pick.id === 'prodOpen') {
        vscode.env.openExternal(
          await vscode.env.asExternalUri(vscode.Uri.parse(`http://127.0.0.1:${prodPort}/`))
        );
        return;
      }

      if (pick.id === 'prodStart') {
        const script = await requireScript(cwd, 'scriptProdStart', 'Production start', 'stack:up');
        if (!script) return;
        runScriptWithEnvInTerminal('Project Production Start', cwd, script, getProjectNetworkEnv());
        void refresh();
      ...
      if (pick.id === 'prodStop') {
        const script = await requireScript(cwd, 'scriptProdStop', 'Production stop', 'stack:down');
        if (!script) return;
      ...
      if (pick.id === 'prodRebuild') {
        const script = await requireScript(cwd, 'scriptProdRebuild', 'Production rebuild', 'stack:rebuild');
        if (!script) return;
        runScriptWithEnvInTerminal('Project Production Rebuild', cwd, script, getProjectNetworkEnv());
        setTimeout(() => void refresh(), 4_000);
        return;
      }

      if (pick.id === 'prodLogs') {
        const script = await requireScript(cwd, 'scriptProdLogs', 'Production logs');
        if (!script) return;
      ...
      if (pick.id === 'prodStatus') {
        const script = await requireScript(cwd, 'scriptProdStatus', 'Production status', 'stack:status');
        if (!script) return;
    ...
    vscode.commands.registerCommand('devkit.deployProjectContracts', async () => {
      ...
      const script = await requireScript(deployDir, 'scriptContractsDeploy', 'Contracts deploy');
      if (!script) return;

      runScriptWithEnvInTerminal('Project Contracts Deploy', deployDir, script, getProjectNetworkEnv());
    }),
    ...
    vscode.commands.registerCommand('devkit.startDev', async () => {
      ...
      await runManagedScript(DEV_PROCESS_ID, 'Project Dev Server', cwd, devScript, getProjectNetworkEnv());
  ...
  context.subscriptions.push(nodeRunningState.onDidChange(() => void refresh()));
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('devkit.refreshStatus', () => void refresh())
  );
}
