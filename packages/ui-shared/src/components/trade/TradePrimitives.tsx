...
function TokenVisual({ token, size = 'h-5 w-5' }: { token: BaseToken | undefined; size?: string }) {
  if (token?.iconUrl) {
    return <img src={token.iconUrl} alt="" className={`${size} rounded-full object-contain shadow-md`} onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }} />;
  }

  return <div className={`flex ${size} items-center justify-center rounded-full bg-white/10 text-[8px] font-black uppercase text-text-secondary`}>{token?.symbol?.[0] ?? '?'}</div>;
}
    ...
    <div ref={rootRef} className={`relative ${open ? 'z-[220]' : 'z-[120]'}`}>
      ...
      >
        <div className="flex min-w-0 items-center gap-2.5">
          ...
          <span className="truncate text-[11px] font-black uppercase tracking-[0.16em] text-white">{selectedToken?.symbol ?? 'Token'}</span>
        </div>
        <span className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-[9px] text-text-secondary/45 transition-transform duration-200" aria-hidden="true">{open ? '▲' : '▼'}</span>
      </button>
              ...
              {tokens.map((entry, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1 truncate text-[11px] font-black uppercase tracking-[0.16em]">{entry.symbol}</div>
                  </button>
                );
              })}
            </div>
  ...
  return (
    <div className="rounded-[1.35rem] border border-white/5 bg-bg-secondary/40 p-4 shadow-lg transition-all duration-300 focus-within:border-accent/40 focus-within:bg-bg-secondary/60">
      ...
      <div className="flex items-center gap-3">
        {readonlyAmount ? (
          <div className="flex-1 font-mono text-[1.7rem] font-black tracking-tight text-white tabular-nums">{amount || '0.0'}</div>
        ) : (
          <Input
            type="text"
            placeholder="0.0"
        ...
        <TradeTokenPicker tokens={tokens} selectedIndex={selectedIndex} onTokenChange={onTokenChange} />
      </div>
    </div>
  );
}

export function TradeSummaryGrid({ items }: { items: Array<{ label: string; value: string; tone?: 'default' | 'accent' }> }) {
  return (
    <div className="grid gap-3 rounded-[1.35rem] border border-white/5 bg-white/5 p-4 shadow-inner sm:grid-cols-2 lg:grid-cols-4">
      {items.map((item) => (
        <div key={item.label} className="flex flex-col gap-1">
          <div className="text-[8px] font-black uppercase tracking-[0.2em] text-text-secondary/55 italic">{item.label}</div>
          <div className={`truncate text-[11px] font-black tracking-widest uppercase ${item.tone === 'accent' ? 'text-accent' : 'text-white'}`}>{item.value}</div>
        </div>
      ))}
    </div>
  );
...
export function TradeActionBar({ children }: { children: ReactNode }) {
  return <div className="mt-5 flex flex-col items-stretch gap-3 rounded-[1.35rem] border border-white/5 bg-bg-secondary/40 p-4 shadow-xl sm:flex-row sm:items-center sm:justify-between">{children}</div>;
}
