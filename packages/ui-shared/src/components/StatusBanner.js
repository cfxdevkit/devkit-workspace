...
export function StatusBanner({ message, tone = 'accent', className = '', textClassName = '' }) {
    const palette = TONE_CLASSES[tone];
    return (_jsxs("div", { className: `flex items-center gap-3 rounded-xl border px-4 py-2.5 animate-fade-in ${palette.container} ${className}`, children: [_jsx("div", { className: `h-1.5 w-1.5 rounded-full ${palette.dot}` }), _jsx("p", { className: `text-[9px] font-black uppercase tracking-[0.15em] ${textClassName}`, children: message })] }));
}
