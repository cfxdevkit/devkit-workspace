  ...
  if (!isConnected) {
    return (
      <div className="card">
        ...
        <p className="text-sm text-text-secondary">Connect wallet to use faucet</p>
      </div>
    );
  }
      ...
      <div className="flex items-center gap-3 mb-2">
        ...
        {faucetBalance != null && (
          <span className="bg-success/10 text-success px-2 py-0.5 rounded-full text-xs font-semibold whitespace-nowrap border border-success/20">
            {faucetBalance.toLocaleString(undefined, { maximumFractionDigits: 0 })} CFX available
          </span>
        )}
      </div>
      <p className="text-sm text-text-secondary mb-4">Fund your wallet with test CFX from the local server</p>
