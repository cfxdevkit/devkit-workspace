...
import { codeServerProxyBasePlugin } from '../vite/code-server-proxy-base';
  ...
  return {
    ...
    configureServer(server: ViteDevServer) {
      server.middlewares.use(async (req: IncomingMessage, res: ServerResponse, next: Connect.NextFunction) => {
        ...
        const send = (status: number, body: object) => { res.writeHead(status); res.end(JSON.stringify(body)); };
        ...
        try {
          ...
          if (req.url === '/api/auth/verify' && req.method === 'POST') {
            ...
            const { message, signature } = JSON.parse(Buffer.concat(chunks).toString());
            ...
            if (!pendingNonces.has(parsed.nonce as string)) return send(400, { error: 'Invalid or expired nonce' });
            pendingNonces.delete(parsed.nonce as string);

            if (parsed.issuedAt) {
              const age = Date.now() - new Date(parsed.issuedAt as string).getTime();
              if (age > 5 * 60_000) return send(400, { error: 'Message expired' });
            }

            const recovered = await recoverAddress(message, signature);
            if (recovered.toLowerCase() !== (parsed.address as string).toLowerCase()) return send(401, { error: 'Signature mismatch' });

            const sid = randomBytes(32).toString('hex');
            sessions.set(sid, { address: parsed.address as string, chainId: parsed.chainId as number });
...
export default defineConfig({
  base: proxyBase ?? './',
  plugins: [react(), codeServerProxyBasePlugin(proxyBase, passthroughPrefixes), siweAuthPlugin()].filter(Boolean),
  resolve: {
