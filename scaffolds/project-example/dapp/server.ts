...
function setSessionCookie(res: ServerResponse, sessionId: string): void {
  res.setHeader('Set-Cookie', `${SESSION_COOKIE}=${sessionId}; HttpOnly; SameSite=Strict; Path=/; Max-Age=${SESSION_MAX_AGE}`);
}
...
function parseSiweMessage(message: string): { address?: string; nonce?: string; chainId?: number; issuedAt?: string } {
  const lines = message.split('\n');
  const result: { address?: string; nonce?: string; chainId?: number; issuedAt?: string } = {};
...
const server = createServer(async (req: IncomingMessage, res: ServerResponse) => {
  ...
  try {
    if (pathname === '/health' && req.method === 'GET') {
      return sendJson(res, 200, { ok: true, server: 'cfxdevkit-project-example' });
    }

    if (pathname.startsWith('/devkit/')) {
      return proxyJsonRequest(req, res, `${DEVKIT_URL}${pathname.slice('/devkit'.length)}${url.search}`);
    }
    ...
    if (pathname === '/api/auth/verify' && req.method === 'POST') {
      const { message, signature } = JSON.parse(await readBody(req)) as { message?: string; signature?: string };
      ...
      if (parsed.issuedAt && Date.now() - new Date(parsed.issuedAt).getTime() > 5 * 60_000) {
      ...
      try {
        ...
        return sendJson(res, 400, { error: 'Signature recovery failed', detail: (error as Error).message });
      }
      ...
      sessions.set(sessionId, { address: parsed.address, chainId: parsed.chainId ?? 0 });
    ...
    if (pathname === '/api/auth/me' && req.method === 'GET') {
      ...
      return session
        ? sendJson(res, 200, { address: session.address, chainId: session.chainId })
    ...
    if (pathname === '/api/contracts/deployed' && req.method === 'GET') {
      try {
        ...
        if (!response.ok) return sendJson(res, response.status, { error: 'devkit error' });
        ...
        return sendJson(res, 503, { error: 'devkit unreachable', detail: (error as Error).message });
      }
    }

    // General /api/* proxy → devkit (accounts/faucet, accounts/fund, etc.)
    if (pathname.startsWith('/api/')) {
      return proxyJsonRequest(req, res, `${DEVKIT_URL}${pathname}${url.search}`);
    }
    ...
    sendJson(res, 500, { error: 'Internal server error', detail: (error as Error).message });
  }
});
