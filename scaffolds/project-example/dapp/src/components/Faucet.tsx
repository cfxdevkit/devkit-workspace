...
export function Faucet() {
  ...
  const refreshFaucetBalance = useCallback(async () => {
    try {
      const r = await fetch(appUrl('devkit/api/accounts/faucet'), { signal: AbortSignal.timeout(5000) });
      if (!r.ok) return;
  ...
  if (!isConnected) {
    return (
      <div className="rounded-xl border border-white/5 bg-bg-secondary/40 p-4 backdrop-blur-2xl">
        <h2 className="text-[10px] font-black tracking-widest text-text-secondary/60 uppercase mb-2 italic">Treasury Access</h2>
        <p className="text-[10px] text-text-secondary/30 font-bold uppercase italic tracking-tighter">Connect wallet to authorize emission.</p>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-2xl border border-white/5 bg-bg-secondary/40 p-5 backdrop-blur-2xl transition-all hover:bg-bg-secondary/60">
      <div className="flex items-center justify-between gap-4 mb-4">
        <div>
          <h2 className="text-xs font-black tracking-widest text-white uppercase leading-none">Faucet</h2>
          <p className="mt-1 text-[8px] font-black uppercase tracking-[0.2em] text-text-secondary/20 italic">On-demand treasury.</p>
        </div>
        {faucetBalance != null && (
          <div className="flex items-center gap-1.5 px-2 py-1 rounded bg-success/5 border border-success/10 shadow-sm italic">
            ...
            <span className="text-[8px] font-bold uppercase tracking-widest text-success/60">
              {faucetBalance.toLocaleString(undefined, { maximumFractionDigits: 0 })} PL
            </span>
          </div>
        )}
      </div>
        ...
        {AMOUNTS.map((amt) => {
          ...
          return (
            <button
              ...
              title={insufficient ? 'Insufficient faucet balance' : `Fund ${amt} CFX`}
            >
              <div className="relative font-mono text-[10px] font-black text-white/90 tracking-widest">{amt} CFX</div>
            </button>
          );
        })}
      </div>
