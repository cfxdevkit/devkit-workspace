    ...
    return (
      <div className="flex items-center gap-2 p-1 pl-3 h-10 rounded-xl bg-warning/5 border border-warning/20 backdrop-blur-md">
        <div className="flex items-center gap-2">
          ...
          <span className="font-mono text-[11px] font-bold text-warning">{shortAddress}</span>
        </div>
        <div className="w-[1px] h-4 bg-warning/20 mx-1" />
        {isSigning ? (
          <span className="text-warning/60 text-[10px] font-bold uppercase tracking-widest px-2 animate-pulse">Signing…</span>
        ) : (
          <button
            type="button"
            onClick={() => void signIn()}
      ...
      <div className="group relative flex items-center gap-0 h-11 rounded-2xl bg-bg-secondary/40 border border-border/50 backdrop-blur-xl shadow-lg transition-all hover:border-border-hover/80 hover:bg-bg-secondary/60">
        ...
        >
          <div className="flex items-center gap-2">
             ...
             <span className="font-mono text-xs font-bold text-text-primary tracking-tight">{shortAddress}</span>
          </div>
          <div className="text-[9px] font-bold text-text-secondary uppercase tracking-[0.1em] mt-0.5 flex items-center gap-1">
             {copied ? (
               <span className="text-success flex items-center gap-1">
                 <svg className="w-2.5 h-2.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true" focusable="false"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M5 13l4 4L19 7" /></svg>
                 Copied
               </span>
             ) : (
               <span className="flex items-center gap-1 transition-colors group-hover:text-accent">
                 <svg className="w-2.5 h-2.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true" focusable="false"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M8 7v8a2 2 0 002 2h6M8 7V5a2 2 0 012-2h4.586a1 1 0 01.707.293l4.414 4.414a1 1 0 01.293.707V15a2 2 0 01-2 2h-2" /></svg>
                 Click to Copy
               </span>
             )}
          </div>
        ...
        {balance && (
          <div className="hidden sm:flex flex-col items-end px-4 h-full justify-center">
            <div className="text-[9px] font-bold text-text-secondary uppercase tracking-[0.15em] leading-none mb-1">Available</div>
            <div className="flex items-baseline gap-1">
              ...
              <span className="text-[10px] font-bold text-accent tracking-tighter">CFX</span>
            </div>
          </div>
        )}
