...
export const TRACKING_PATH = resolve(__dirname, '..', 'deployments', 'contracts.json');
export const ARTIFACT_PATH = resolve(__dirname, '..', 'dapp', 'src', 'generated', 'contracts-addresses.ts');
...
function ensureNetwork(state, network, chainId) {
  ...
  if (!state.networks[network].contracts || typeof state.networks[network].contracts !== 'object') {
...
export function upsertDeployment(state, entry) {
  ...
  if (!Number.isFinite(chainId) || chainId <= 0) {
    throw new Error(`Invalid chainId for ${entry.contractName}: ${entry.chainId}`);
  }
...
export function normalizeChainId(contract) {
  if (typeof contract?.chainId === 'number' && Number.isFinite(contract.chainId)) {
...
export function updateFromDeployedContracts(state, contracts, source = 'devkit') {
  for (const contract of contracts) {
    if (!contract || typeof contract.address !== 'string' || typeof contract.name !== 'string') continue;
...
function buildArtifact(map, generatedBy) {
...
export function writeFrontendArtifactFromTracking(
  ...
  writeFileSync(artifactPath, buildArtifact(map, generatedBy), 'utf8');
}
